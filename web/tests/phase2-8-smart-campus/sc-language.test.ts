...
import { ScLanguageService } from '@/features/smart-campus/services/sclanguageservice.service';

describe('ScLanguageService', () => {
  let service: ScLanguageService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScLanguageService(mockSupabase);
  });

  describe('setLanguage', () => {
    it('should setLanguage successfully', async () => {
      const result = await service.setLanguage('user-1', 'en');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.setLanguage('user-1', 'en');
      const promise2 = service.setLanguage('user-1', 'en');
