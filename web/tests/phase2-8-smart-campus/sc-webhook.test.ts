...
import { ScWebhookService } from '@/features/smart-campus/services/scwebhookservice.service';

describe('ScWebhookService', () => {
  let service: ScWebhookService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScWebhookService(mockSupabase);
  });

  describe('createWebhook', () => {
    it('should createWebhook successfully', async () => {
      const result = await service.createWebhook('school-1', { url: 'https://example.com', events: ['create'] });
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.createWebhook('school-1', { url: 'https://example.com', events: ['create'] });
      const promise2 = service.createWebhook('school-1', { url: 'https://example.com', events: ['create'] });
