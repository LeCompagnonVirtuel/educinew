...
import { ScExportService } from '@/features/smart-campus/services/scexportservice.service';

describe('ScExportService', () => {
  let service: ScExportService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScExportService(mockSupabase);
  });

  describe('exportData', () => {
    it('should exportData successfully', async () => {
      const result = await service.exportData('school-1', 'csv');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.exportData('school-1', 'csv');
      const promise2 = service.exportData('school-1', 'csv');
