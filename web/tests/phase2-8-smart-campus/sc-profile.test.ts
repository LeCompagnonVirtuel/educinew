...
import { ScProfileService } from '@/features/smart-campus/services/scprofileservice.service';

describe('ScProfileService', () => {
  let service: ScProfileService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScProfileService(mockSupabase);
  });

  describe('getProfile', () => {
    it('should getProfile successfully', async () => {
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.getProfile('user-1');
      const promise2 = service.getProfile('user-1');
