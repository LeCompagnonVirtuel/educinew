...
import { ScCameraManagementService } from '@/features/smart-campus/services/sccameramanagementservice.service';

describe('ScCameraManagementService', () => {
  let service: ScCameraManagementService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScCameraManagementService(mockSupabase);
  });

  describe('getCameraFeed', () => {
    it('should getCameraFeed successfully', async () => {
      const result = await service.getCameraFeed('camera-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.getCameraFeed('camera-1');
      const promise2 = service.getCameraFeed('camera-1');
