...
import { ScFixService } from '@/features/smart-campus/services/scfixservice.service';

describe('ScFixService', () => {
  let service: ScFixService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScFixService(mockSupabase);
  });

  describe('fixData', () => {
    it('should fixData successfully', async () => {
      const result = await service.fixData('school-1', 'corrupted-records');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.fixData('school-1', 'corrupted-records');
      const promise2 = service.fixData('school-1', 'corrupted-records');
