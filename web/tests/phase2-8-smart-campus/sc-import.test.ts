...
import { ScImportService } from '@/features/smart-campus/services/scimportservice.service';

describe('ScImportService', () => {
  let service: ScImportService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScImportService(mockSupabase);
  });

  describe('importData', () => {
    it('should importData successfully', async () => {
      const result = await service.importData('school-1', 'csv', 'data');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.importData('school-1', 'csv', 'data');
      const promise2 = service.importData('school-1', 'csv', 'data');
