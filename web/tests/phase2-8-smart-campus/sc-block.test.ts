...
import { ScBlockService } from '@/features/smart-campus/services/scblockservice.service';

describe('ScBlockService', () => {
  let service: ScBlockService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScBlockService(mockSupabase);
  });
  ...
  describe('block', () => {
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.block('user-1', 'user-2');
      const promise2 = service.block('user-1', 'user-2');
