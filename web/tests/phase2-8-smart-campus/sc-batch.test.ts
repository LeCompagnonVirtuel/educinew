...
import { ScBatchService } from '@/features/smart-campus/services/scbatchservice.service';

describe('ScBatchService', () => {
  let service: ScBatchService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScBatchService(mockSupabase);
  });

  describe('processBatch', () => {
    it('should processBatch successfully', async () => {
      const result = await service.processBatch('school-1', 'process', []);
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.processBatch('school-1', 'process', []);
      const promise2 = service.processBatch('school-1', 'process', []);
