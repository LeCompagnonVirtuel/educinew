...
import { ScConsentService } from '@/features/smart-campus/services/scconsentservice.service';

describe('ScConsentService', () => {
  let service: ScConsentService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScConsentService(mockSupabase);
  });

  describe('recordConsent', () => {
    it('should recordConsent successfully', async () => {
      const result = await service.recordConsent('user-1', 'data-collection');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.recordConsent('user-1', 'data-collection');
      const promise2 = service.recordConsent('user-1', 'data-collection');
