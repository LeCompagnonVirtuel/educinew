...
import { ScGlobalSearchService } from '@/features/smart-campus/services/scglobalsearchservice.service';

describe('ScGlobalSearchService', () => {
  let service: ScGlobalSearchService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScGlobalSearchService(mockSupabase);
  });

  describe('search', () => {
    it('should search successfully', async () => {
      const result = await service.search('school-1', 'test query');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.search('school-1', 'test query');
      const promise2 = service.search('school-1', 'test query');
