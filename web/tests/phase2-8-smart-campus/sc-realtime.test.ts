...
import { ScRealtimeService } from '@/features/smart-campus/services/screaltimeservice.service';

describe('ScRealtimeService', () => {
  let service: ScRealtimeService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScRealtimeService(mockSupabase);
  });

  describe('subscribeToChannel', () => {
    it('should subscribeToChannel successfully', async () => {
      const result = await service.subscribeToChannel('school-1', 'events');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.subscribeToChannel('school-1', 'events');
      const promise2 = service.subscribeToChannel('school-1', 'events');
