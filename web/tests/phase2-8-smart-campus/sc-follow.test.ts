...
import { ScFollowService } from '@/features/smart-campus/services/scfollowservice.service';

describe('ScFollowService', () => {
  let service: ScFollowService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScFollowService(mockSupabase);
  });
  ...
  describe('follow', () => {
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.follow('user-1', 'user-2');
      const promise2 = service.follow('user-1', 'user-2');
