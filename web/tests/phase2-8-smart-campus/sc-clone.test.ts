...
import { ScCloneService } from '@/features/smart-campus/services/sccloneservice.service';

describe('ScCloneService', () => {
  let service: ScCloneService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScCloneService(mockSupabase);
  });

  describe('clone', () => {
    it('should clone successfully', async () => {
      const result = await service.clone('resource-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.clone('resource-1');
      const promise2 = service.clone('resource-1');
