...
import { ScRateLimitService } from '@/features/smart-campus/services/scratelimitservice.service';

describe('ScRateLimitService', () => {
  let service: ScRateLimitService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScRateLimitService(mockSupabase);
  });

  describe('checkRateLimit', () => {
    it('should checkRateLimit successfully', async () => {
      const result = await service.checkRateLimit('user-1', 'api');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.checkRateLimit('user-1', 'api');
      const promise2 = service.checkRateLimit('user-1', 'api');
