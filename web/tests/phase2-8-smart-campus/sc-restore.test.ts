...
import { ScRestoreService } from '@/features/smart-campus/services/screstoreservice.service';

describe('ScRestoreService', () => {
  let service: ScRestoreService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScRestoreService(mockSupabase);
  });

  describe('restore', () => {
    it('should restore successfully', async () => {
      const result = await service.restore('user-1', 'resource-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.restore('user-1', 'resource-1');
      const promise2 = service.restore('user-1', 'resource-1');
