...
import { ScSeedService } from '@/features/smart-campus/services/scseedservice.service';

describe('ScSeedService', () => {
  let service: ScSeedService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScSeedService(mockSupabase);
  });

  describe('seedData', () => {
    it('should seedData successfully', async () => {
      const result = await service.seedData('school-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.seedData('school-1');
      const promise2 = service.seedData('school-1');
