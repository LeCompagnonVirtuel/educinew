...
import { ScOfflineSyncService } from '@/features/smart-campus/services/scofflinesyncservice.service';

describe('ScOfflineSyncService', () => {
  let service: ScOfflineSyncService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScOfflineSyncService(mockSupabase);
  });

  describe('syncData', () => {
    it('should syncData successfully', async () => {
      const result = await service.syncData('school-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.syncData('school-1');
      const promise2 = service.syncData('school-1');
