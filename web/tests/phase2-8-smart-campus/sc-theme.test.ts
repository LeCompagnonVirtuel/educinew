...
import { ScThemeService } from '@/features/smart-campus/services/scthemeservice.service';

describe('ScThemeService', () => {
  let service: ScThemeService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScThemeService(mockSupabase);
  });

  describe('setTheme', () => {
    it('should setTheme successfully', async () => {
      const result = await service.setTheme('user-1', 'dark');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.setTheme('user-1', 'dark');
      const promise2 = service.setTheme('user-1', 'dark');
