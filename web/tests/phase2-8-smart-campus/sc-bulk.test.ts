...
import { ScBulkService } from '@/features/smart-campus/services/scbulkservice.service';

describe('ScBulkService', () => {
  let service: ScBulkService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScBulkService(mockSupabase);
  });

  describe('bulkOperation', () => {
    it('should bulkOperation successfully', async () => {
      const result = await service.bulkOperation('school-1', 'update', []);
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.bulkOperation('school-1', 'update', []);
      const promise2 = service.bulkOperation('school-1', 'update', []);
