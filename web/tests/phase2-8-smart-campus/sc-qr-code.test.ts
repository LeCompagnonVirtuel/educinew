...
import { ScQrCodeService } from '@/features/smart-campus/services/scqrcodeservice.service';

describe('ScQrCodeService', () => {
  let service: ScQrCodeService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScQrCodeService(mockSupabase);
  });

  describe('generateQrCode', () => {
    it('should generateQrCode successfully', async () => {
      const result = await service.generateQrCode('resource-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.generateQrCode('resource-1');
      const promise2 = service.generateQrCode('resource-1');
