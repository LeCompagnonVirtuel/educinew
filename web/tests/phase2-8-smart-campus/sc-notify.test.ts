...
import { ScNotifyService } from '@/features/smart-campus/services/scnotifyservice.service';

describe('ScNotifyService', () => {
  let service: ScNotifyService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScNotifyService(mockSupabase);
  });

  describe('notify', () => {
    it('should notify successfully', async () => {
      const result = await service.notify('user-1', 'Test notification');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.notify('user-1', 'Test notification');
      const promise2 = service.notify('user-1', 'Test notification');
