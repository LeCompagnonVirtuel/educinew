...
import { ScBackupService } from '@/features/smart-campus/services/scbackupservice.service';

describe('ScBackupService', () => {
  let service: ScBackupService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScBackupService(mockSupabase);
  });

  describe('createBackup', () => {
    it('should createBackup successfully', async () => {
      const result = await service.createBackup('school-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.createBackup('school-1');
      const promise2 = service.createBackup('school-1');
