...
import { ScIotManagementService } from '@/features/smart-campus/services/sciotmanagementservice.service';

describe('ScIotManagementService', () => {
  let service: ScIotManagementService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScIotManagementService(mockSupabase);
  });

  describe('registerDevice', () => {
    it('should registerDevice successfully', async () => {
      const result = await service.registerDevice('school-1', { name: 'Sensor-1', type: 'temperature' });
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.registerDevice('school-1', { name: 'Sensor-1', type: 'temperature' });
      const promise2 = service.registerDevice('school-1', { name: 'Sensor-1', type: 'temperature' });
