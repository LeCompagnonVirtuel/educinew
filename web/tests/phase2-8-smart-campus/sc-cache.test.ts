...
import { ScCacheService } from '@/features/smart-campus/services/sccacheservice.service';

describe('ScCacheService', () => {
  let service: ScCacheService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScCacheService(mockSupabase);
  });

  describe('getCache', () => {
    it('should getCache successfully', async () => {
      const result = await service.getCache('key-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.getCache('key-1');
      const promise2 = service.getCache('key-1');
