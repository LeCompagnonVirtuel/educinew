...
import { ScPublishService } from '@/features/smart-campus/services/scpublishservice.service';

describe('ScPublishService', () => {
  let service: ScPublishService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScPublishService(mockSupabase);
  });

  describe('publish', () => {
    it('should publish successfully', async () => {
      const result = await service.publish('user-1', 'resource-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.publish('user-1', 'resource-1');
      const promise2 = service.publish('user-1', 'resource-1');
