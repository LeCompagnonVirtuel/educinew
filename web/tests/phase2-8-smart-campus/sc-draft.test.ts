...
import { ScDraftService } from '@/features/smart-campus/services/scdraftservice.service';

describe('ScDraftService', () => {
  let service: ScDraftService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScDraftService(mockSupabase);
  });

  describe('createDraft', () => {
    it('should createDraft successfully', async () => {
      const result = await service.createDraft('user-1', 'resource-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.createDraft('user-1', 'resource-1');
      const promise2 = service.createDraft('user-1', 'resource-1');
