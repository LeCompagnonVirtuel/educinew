...
import { ScPinService } from '@/features/smart-campus/services/scpinservice.service';

describe('ScPinService', () => {
  let service: ScPinService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScPinService(mockSupabase);
  });

  describe('pin', () => {
    it('should pin successfully', async () => {
      const result = await service.pin('user-1', 'resource-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.pin('user-1', 'resource-1');
      const promise2 = service.pin('user-1', 'resource-1');
