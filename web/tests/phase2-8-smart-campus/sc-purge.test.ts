...
import { ScPurgeService } from '@/features/smart-campus/services/scpurgeservice.service';

describe('ScPurgeService', () => {
  let service: ScPurgeService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScPurgeService(mockSupabase);
  });

  describe('purgeData', () => {
    it('should purgeData successfully', async () => {
      const result = await service.purgeData('school-1', 'old-logs');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.purgeData('school-1', 'old-logs');
      const promise2 = service.purgeData('school-1', 'old-logs');
