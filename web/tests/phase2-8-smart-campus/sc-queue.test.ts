...
import { ScQueueService } from '@/features/smart-campus/services/scqueueservice.service';

describe('ScQueueService', () => {
  let service: ScQueueService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScQueueService(mockSupabase);
  });

  describe('addToQueue', () => {
    it('should addToQueue successfully', async () => {
      const result = await service.addToQueue('queue-1', { task: 'process' });
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.addToQueue('queue-1', { task: 'process' });
      const promise2 = service.addToQueue('queue-1', { task: 'process' });
