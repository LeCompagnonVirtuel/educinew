...
import { ScOptimizeService } from '@/features/smart-campus/services/scoptimizeservice.service';

describe('ScOptimizeService', () => {
  let service: ScOptimizeService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScOptimizeService(mockSupabase);
  });

  describe('optimize', () => {
    it('should optimize successfully', async () => {
      const result = await service.optimize('school-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.optimize('school-1');
      const promise2 = service.optimize('school-1');
