...
import { ScRatingService } from '@/features/smart-campus/services/scratingservice.service';

describe('ScRatingService', () => {
  let service: ScRatingService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScRatingService(mockSupabase);
  });

  describe('rate', () => {
    it('should rate successfully', async () => {
      const result = await service.rate('user-1', 'resource-1', 5);
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.rate('user-1', 'resource-1', 5);
      const promise2 = service.rate('user-1', 'resource-1', 5);
