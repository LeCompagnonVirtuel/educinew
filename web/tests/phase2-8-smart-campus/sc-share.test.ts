...
import { ScShareService } from '@/features/smart-campus/services/scshareservice.service';

describe('ScShareService', () => {
  let service: ScShareService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScShareService(mockSupabase);
  });

  describe('share', () => {
    it('should share successfully', async () => {
      const result = await service.share('user-1', 'resource-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.share('user-1', 'resource-1');
      const promise2 = service.share('user-1', 'resource-1');
