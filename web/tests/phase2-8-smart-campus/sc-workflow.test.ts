...
import { ScWorkflowService } from '@/features/smart-campus/services/scworkflowservice.service';

describe('ScWorkflowService', () => {
  let service: ScWorkflowService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScWorkflowService(mockSupabase);
  });

  describe('createWorkflow', () => {
    it('should createWorkflow successfully', async () => {
      const result = await service.createWorkflow('school-1', { name: 'Approval', steps: [] });
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.createWorkflow('school-1', { name: 'Approval', steps: [] });
      const promise2 = service.createWorkflow('school-1', { name: 'Approval', steps: [] });
