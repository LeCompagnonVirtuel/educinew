...
import { ScPredictiveService } from '@/features/smart-campus/services/scpredictiveservice.service';

describe('ScPredictiveService', () => {
  let service: ScPredictiveService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScPredictiveService(mockSupabase);
  });

  describe('predict', () => {
    it('should predict successfully', async () => {
      const result = await service.predict('school-1', 'enrollment');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.predict('school-1', 'enrollment');
      const promise2 = service.predict('school-1', 'enrollment');
