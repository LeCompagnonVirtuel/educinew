...
import { ScLikeService } from '@/features/smart-campus/services/sclikeservice.service';

describe('ScLikeService', () => {
  let service: ScLikeService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScLikeService(mockSupabase);
  });

  describe('like', () => {
    it('should like successfully', async () => {
      const result = await service.like('user-1', 'resource-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.like('user-1', 'resource-1');
      const promise2 = service.like('user-1', 'resource-1');
