...
import { ScVacuumService } from '@/features/smart-campus/services/scvacuumservice.service';

describe('ScVacuumService', () => {
  let service: ScVacuumService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScVacuumService(mockSupabase);
  });

  describe('vacuum', () => {
    it('should vacuum successfully', async () => {
      const result = await service.vacuum('school-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.vacuum('school-1');
      const promise2 = service.vacuum('school-1');
