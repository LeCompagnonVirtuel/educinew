...
import { ScReindexService } from '@/features/smart-campus/services/screindexservice.service';

describe('ScReindexService', () => {
  let service: ScReindexService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScReindexService(mockSupabase);
  });

  describe('reindex', () => {
    it('should reindex successfully', async () => {
      const result = await service.reindex('school-1');
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.reindex('school-1');
      const promise2 = service.reindex('school-1');
