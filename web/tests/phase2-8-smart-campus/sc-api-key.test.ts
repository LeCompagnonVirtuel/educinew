...
import { ScApiKeyService } from '@/features/smart-campus/services/scapikeyservice.service';

describe('ScApiKeyService', () => {
  let service: ScApiKeyService;
  const mockSupabase = {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new ScApiKeyService(mockSupabase);
  });

  describe('generateApiKey', () => {
    it('should generateApiKey successfully', async () => {
    ...
    it('should handle concurrent requests', async () => {
      const promise1 = service.generateApiKey('user-1');
      const promise2 = service.generateApiKey('user-1');
