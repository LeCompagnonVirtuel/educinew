...
import { LxpStudyGroupService } from '@/features/lxp/services/lxp-study-group.service';
...
describe('LxpStudyGroupService', () => {
  let service: LxpStudyGroupService;

  beforeEach(() => {
    vi.clearAllMocks();
    mockSupabase.data = null;
    mockSupabase.error = null;
    service = new LxpStudyGroupService(mockSupabase as never);
  });

  describe('GetStudyGroup', () => {
    it('should getStudyGroup study group successfully', async () => {
      mockSupabase.data = { id: 'test-1' };
      mockSupabase.single.mockResolvedValue({ data: mockSupabase.data, error: null });
      const result = await service.GetStudyGroup('test-1');
      expect(result).toBeDefined();
    });

    it('should return null when study group not found', async () => {
      mockSupabase.single.mockResolvedValue({ data: null, error: null });
      const result = await service.GetStudyGroup('nonexistent');
      expect(result).toBeNull();
    });

    it('should handle database errors during getStudyGroup', async () => {
      mockSupabase.single.mockResolvedValue({ data: null, error: { message: 'db error' } });
      await expect(service.GetStudyGroup('test-1')).rejects.toThrow();
    });

    it('should validate input parameters for getStudyGroup', async () => {
      await expect(service.GetStudyGroup('')).rejects.toThrow();
    });
  });
  describe('CreateStudyGroup', () => {
    it('should createStudyGroup study group successfully', async () => {
      mockSupabase.data = { id: 'test-1' };
      mockSupabase.single.mockResolvedValue({ data: mockSupabase.data, error: null });
      const result = await service.CreateStudyGroup('test-1');
      expect(result).toBeDefined();
    });

    it('should return null when study group not found', async () => {
      mockSupabase.single.mockResolvedValue({ data: null, error: null });
      const result = await service.CreateStudyGroup('nonexistent');
      expect(result).toBeNull();
    });

    it('should handle database errors during createStudyGroup', async () => {
      mockSupabase.single.mockResolvedValue({ data: null, error: { message: 'db error' } });
      await expect(service.CreateStudyGroup('test-1')).rejects.toThrow();
    });

    it('should validate input parameters for createStudyGroup', async () => {
      await expect(service.CreateStudyGroup('')).rejects.toThrow();
    });
  });
  describe('UpdateStudyGroup', () => {
    it('should updateStudyGroup study group successfully', async () => {
      mockSupabase.data = { id: 'test-1' };
      mockSupabase.single.mockResolvedValue({ data: mockSupabase.data, error: null });
      const result = await service.UpdateStudyGroup('test-1');
      expect(result).toBeDefined();
    });

    it('should return null when study group not found', async () => {
      mockSupabase.single.mockResolvedValue({ data: null, error: null });
      const result = await service.UpdateStudyGroup('nonexistent');
      expect(result).toBeNull();
    });

    it('should handle database errors during updateStudyGroup', async () => {
      mockSupabase.single.mockResolvedValue({ data: null, error: { message: 'db error' } });
      await expect(service.UpdateStudyGroup('test-1')).rejects.toThrow();
    });

    it('should validate input parameters for updateStudyGroup', async () => {
      await expect(service.UpdateStudyGroup('')).rejects.toThrow();
    });
  });
  describe('DeleteStudyGroup', () => {
    it('should deleteStudyGroup study group successfully', async () => {
      mockSupabase.data = { id: 'test-1' };
      mockSupabase.single.mockResolvedValue({ data: mockSupabase.data, error: null });
      const result = await service.DeleteStudyGroup('test-1');
      expect(result).toBeDefined();
    });

    it('should return null when study group not found', async () => {
      mockSupabase.single.mockResolvedValue({ data: null, error: null });
      const result = await service.DeleteStudyGroup('nonexistent');
      expect(result).toBeNull();
    });

    it('should handle database errors during deleteStudyGroup', async () => {
      mockSupabase.single.mockResolvedValue({ data: null, error: { message: 'db error' } });
      await expect(service.DeleteStudyGroup('test-1')).rejects.toThrow();
    });

    it('should validate input parameters for deleteStudyGroup', async () => {
      await expect(service.DeleteStudyGroup('')).rejects.toThrow();
    });
  });
  describe('AddMember', () => {
    it('should addMember study group successfully', async () => {
      mockSupabase.data = { id: 'test-1' };
      mockSupabase.single.mockResolvedValue({ data: mockSupabase.data, error: null });
  ...
  describe('RemoveMember', () => {
    it('should removeMember study group successfully', async () => {
      mockSupabase.data = { id: 'test-1' };
      mockSupabase.single.mockResolvedValue({ data: mockSupabase.data, error: null });
  ...
  describe('GetSession', () => {
    it('should getSession study group successfully', async () => {
      mockSupabase.data = { id: 'test-1' };
      mockSupabase.single.mockResolvedValue({ data: mockSupabase.data, error: null });
  ...
  describe('CreateSession', () => {
    it('should createSession study group successfully', async () => {
      mockSupabase.data = { id: 'test-1' };
      mockSupabase.single.mockResolvedValue({ data: mockSupabase.data, error: null });
  ...
  describe('GetSessions', () => {
    it('should getSessions study group successfully', async () => {
      mockSupabase.data = { id: 'test-1' };
      mockSupabase.single.mockResolvedValue({ data: mockSupabase.data, error: null });
      const result = await service.GetSessions('test-1');
      expect(result).toBeDefined();
    });

    it('should return null when study group not found', async () => {
      mockSupabase.single.mockResolvedValue({ data: null, error: null });
      const result = await service.GetSessions('nonexistent');
      expect(result).toBeNull();
    });

    it('should handle database errors during getSessions', async () => {
      mockSupabase.single.mockResolvedValue({ data: null, error: { message: 'db error' } });
      await expect(service.GetSessions('test-1')).rejects.toThrow();
    });

    it('should validate input parameters for getSessions', async () => {
      await expect(service.GetSessions('')).rejects.toThrow();
    });
  });
  describe('GetGroupProgress', () => {
    it('should getGroupProgress study group successfully', async () => {
      mockSupabase.data = { id: 'test-1' };
      mockSupabase.single.mockResolvedValue({ data: mockSupabase.data, error: null });
