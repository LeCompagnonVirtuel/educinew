...
export class CrudRepositoryImpl<T extends BaseEntity> implements CrudRepository<T> {
  ...
  async softDelete(id: string, schoolId: string): Promise<void> {
    ...
    const { error } = await this.supabase
      ...
      .eq('id', id)
      .eq('school_id', schoolId);
    if (error) throw new Error(`Failed to soft delete ${this.tableName}: ${error.message}`);
  }
  ...
  async restore(id: string, schoolId: string): Promise<void> {
    const { error } = await this.supabase
      ...
      .eq('id', id)
      .eq('school_id', schoolId);
    if (error) throw new Error(`Failed to restore ${this.tableName}: ${error.message}`);
  }
  ...
  async exists(id: string, schoolId: string): Promise<boolean> {
    const { count, error } = await this.supabase
      ...
      .eq('school_id', schoolId)
      .is('deleted_at', null);
    if (error) throw new Error(`Failed to check ${this.tableName}: ${error.message}`);
    return (count || 0) > 0;
  }
}
