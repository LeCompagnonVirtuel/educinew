'use client';

import { useState, useEffect, useCallback } from 'react';
import { EduOSDependencyGraphService } from '../services/eduos-dependency-graph.service';
import { createClient } from '@/lib/supabase/client';
import type { DependencyGraph } from '@educi/types';

export const useEduOSDependencyGraphList = (schoolId: string) => {
  const [items, setItems] = useState<DependencyGraph[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const fetchItems = useCallback(async () => {
    try {
      setLoading(true);
      const supabase = createClient();
      const service = new EduOSDependencyGraphService(supabase);
      const data = await service.listDependencyGraphs(schoolId);
      setItems(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erreur inconnue');
    } finally {
      setLoading(false);
    }
  }, [schoolId]);

  useEffect(() => {
    fetchItems();
  }, [fetchItems]);

  return { items, loading, error, refresh: fetchItems };
};
