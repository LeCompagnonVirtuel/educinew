'use client';

import { useState, useEffect, useCallback } from 'react';
import { EduOSIdentityConsentService } from '../services/eduos-identity-consent.service';
import { createClient } from '@/lib/supabase/client';
import type { IdentityConsent } from '@educi/types';

export const useEduOSIdentityConsentList = (schoolId: string) => {
  const [items, setItems] = useState<IdentityConsent[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const fetchItems = useCallback(async () => {
    try {
      setLoading(true);
      const supabase = createClient();
      const service = new EduOSIdentityConsentService(supabase);
      const data = await service.listIdentityConsents(schoolId);
      setItems(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erreur inconnue');
    } finally {
      setLoading(false);
    }
  }, [schoolId]);

  useEffect(() => {
    fetchItems();
  }, [fetchItems]);

  return { items, loading, error, refresh: fetchItems };
};
