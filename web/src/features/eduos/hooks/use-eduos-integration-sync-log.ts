'use client';

import { useState, useEffect, useCallback } from 'react';
import { EduOSIntegrationSyncLogService } from '../services/eduos-integration-sync-log.service';
import { createClient } from '@/lib/supabase/client';
import type { IntegrationSyncLog } from '@educi/types';

export const useEduOSIntegrationSyncLogList = (schoolId: string) => {
  const [items, setItems] = useState<IntegrationSyncLog[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const fetchItems = useCallback(async () => {
    try {
      setLoading(true);
      const supabase = createClient();
      const service = new EduOSIntegrationSyncLogService(supabase);
      const data = await service.listIntegrationSyncLogs(schoolId);
      setItems(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erreur inconnue');
    } finally {
      setLoading(false);
    }
  }, [schoolId]);

  useEffect(() => {
    fetchItems();
  }, [fetchItems]);

  return { items, loading, error, refresh: fetchItems };
};