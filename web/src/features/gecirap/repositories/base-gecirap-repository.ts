...
export interface GecirapBaseEntity {
  id: string;
  school_id: string;
  created_at: string;
  updated_at: string;
...
export interface GecirapCrudRepository<T extends GecirapBaseEntity> {
...
export class GecirapCrudRepositoryImpl<T extends GecirapBaseEntity>
  implements GecirapCrudRepository<T>
{
  constructor(
    protected supabase: SupabaseClient,
    protected tableName: string,
    protected errorHandler: (message: string) => never = (msg) => {
      throw new GecirapNotFoundError(msg);
    },
  ) {}
...
export function createGecirapCrudRepository<T extends GecirapBaseEntity>(
  supabase: SupabaseClient,
  tableName: string,
): GecirapCrudRepository<T> {
  return new GecirapCrudRepositoryImpl<T>(supabase, tableName);
}
