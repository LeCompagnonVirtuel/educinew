...
import type { OnboardingRepository, OnboardingState, OnboardingData, OnboardingProgress, OnboardingStatus } from '../types';
import { OnboardingNotFoundError, OnboardingConflictError } from '@educi/errors';
...
export function createOnboardingRepository(): OnboardingRepository {
  ...
  return {
    async create(userId: string): Promise<OnboardingState> {
      ...
      if (error) {
        logger.error('Failed to create onboarding', { userId, error: error.message }, 'onboarding');
        throw new OnboardingConflictError('Erreur lors de la création de l\'onboarding');
      }

      logger.info('Onboarding created', { id: data.id, userId }, 'onboarding');
      return {
        id: data.id,
        userId,
    ...
    async update(id: string, data: Partial<OnboardingData>): Promise<OnboardingState> {
      ...
      if (!existing) throw new OnboardingNotFoundError(id);
      ...
      if (error) {
        logger.error('Failed to update onboarding', { id, error: error.message }, 'onboarding');
        throw new OnboardingNotFoundError(id);
      }

      logger.info('Onboarding updated', { id }, 'onboarding');
    ...
    async updateProgress(id: string, progress: Partial<OnboardingProgress>): Promise<void> {
      ...
      if (error) {
        logger.error('Failed to update onboarding progress', { id, error: error.message }, 'onboarding');
        throw new OnboardingNotFoundError(id);
      }
    },
    ...
    async updateStatus(id: string, status: OnboardingStatus): Promise<void> {
      ...
      if (error) {
        logger.error('Failed to update onboarding status', { id, status, error: error.message }, 'onboarding');
        throw new OnboardingNotFoundError(id);
      }
    },
    ...
    async delete(id: string): Promise<void> {
      ...
      if (error) {
        logger.error('Failed to delete onboarding', { id, error: error.message }, 'onboarding');
        throw new OnboardingNotFoundError(id);
      }
    ...
    async findDrafts(userId: string): Promise<OnboardingState[]> {
      const { data, error } = await supabase
        .from('onboarding_drafts')
        .select('*')
        .eq('user_id', userId)
      ...
      return data.map((row: any) => ({
        ...
        data: row.data as unknown as OnboardingData,
        progress: row.progress as unknown as OnboardingProgress,
    ...
    async complete(id: string, schoolId: string): Promise<void> {
      ...
      if (error) {
        logger.error('Failed to complete onboarding', { id, schoolId, error: error.message }, 'onboarding');
        throw new OnboardingNotFoundError(id);
      }

      logger.info('Onboarding completed', { id, schoolId }, 'onboarding');
    },
  };
}
