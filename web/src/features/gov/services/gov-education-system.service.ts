// Government & National Governance Service - EducationSystem
...
export class GovEducationSystemService {
  ...
  async getEducationSystem(schoolId: string, id: string): Promise<EducationSystem> {
    const item = await this.repo.findEducationSystemById(schoolId, id);
    if (!item) throw new GovEducationSystemNotFoundError(id);
    return item;
  }

  async listEducationSystems(schoolId: string, filters?: Record<string, unknown>): Promise<EducationSystem[]> {
  ...
  async updateEducationSystem(schoolId: string, id: string, data: Partial<EducationSystemCreate>): Promise<EducationSystem> {
  ...
  async countEducationSystems(schoolId: string, filters?: Record<string, unknown>): Promise<number> {
    return this.repo.countEducationSystems(schoolId, filters);
  }
}
