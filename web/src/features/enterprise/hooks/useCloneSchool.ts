...
export function useCloneSchool(supabase: any, schoolId: string) {
  ...
  const cloneSchool = useCallback(async (targetSchoolId: string, cloneData: any) => {
    setLoading(true);
    setError(null);
    try {
      const result = await service.cloneSchool(schoolId, targetSchoolId, cloneData);
      setData(result);
      return result;
    } catch (err: any) {
  ...
  return { data, loading, error, cloneSchool };
}
