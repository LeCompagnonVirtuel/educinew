'use client';

import { useState, useEffect, useCallback } from 'react';
import { EntSchedulerConfigService } from '../services/scheduler-config.service';
import { createClient } from '@/lib/supabase/client';
import type { SchedulerConfig } from '@educi/types';

export const useEntSchedulerConfigList = (schoolId: string) => {
  const [items, setItems] = useState<SchedulerConfig[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const fetchItems = useCallback(async () => {
    try {
      setLoading(true);
      const supabase = createClient();
      const service = new EntSchedulerConfigService(supabase);
      const data = await service.listSchedulerConfigs(schoolId);
      setItems(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erreur inconnue');
    } finally {
      setLoading(false);
    }
  }, [schoolId]);

  useEffect(() => { fetchItems(); }, [fetchItems]);

  return { items, loading, error, refresh: fetchItems };
};
