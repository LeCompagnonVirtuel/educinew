...
export function useUnlockEnterpriseUser(supabase: any, schoolId: string) {
  ...
  const unlockUser = useCallback(async (userId: string) => {
    ...
    setError(null);
    try {
      const result = await service.unlockUser(schoolId, userId);
      setData(result);
      return result;
    } catch (err: any) {
  ...
  return { data, loading, error, unlockUser };
}
