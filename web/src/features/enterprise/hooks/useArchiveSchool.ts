...
export function useArchiveSchool(supabase: any, schoolId: string) {
  ...
  const archiveSchool = useCallback(async (targetSchoolId: string) => {
    setLoading(true);
    setError(null);
    try {
      const result = await service.archiveSchool(schoolId, targetSchoolId);
      setData(result);
      return result;
    } catch (err: any) {
  ...
  return { data, loading, error, archiveSchool };
}
