...
export function useBlockSchool(supabase: any, schoolId: string) {
  ...
  const blockSchool = useCallback(async (targetSchoolId: string, reason?: string) => {
    setLoading(true);
    setError(null);
    try {
      const result = await service.blockSchool(schoolId, targetSchoolId, reason);
      setData(result);
      return result;
    } catch (err: any) {
  ...
  return { data, loading, error, blockSchool };
}
