...
export function useSuspendSchool(supabase: any, schoolId: string) {
  ...
  const suspendSchool = useCallback(async (targetSchoolId: string, reason?: string) => {
    setLoading(true);
    setError(null);
    try {
      const result = await service.suspendSchool(schoolId, targetSchoolId, reason);
      setData(result);
      return result;
    } catch (err: any) {
  ...
  return { data, loading, error, suspendSchool };
}
