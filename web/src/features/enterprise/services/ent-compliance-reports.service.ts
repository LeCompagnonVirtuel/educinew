// Enterprise Platform Service - ComplianceReports
...
export class EntComplianceReportService {
  private repo: EnterprisePlatformRepository;
  constructor(private supabase: SupabaseClient) {
    this.repo = new EnterprisePlatformRepository(supabase);
  }
  async getComplianceReport(schoolId: string, id: string): Promise<GenericRecord> {
    const item = await this.repo.findComplianceReportById(schoolId, id);
    if (!item) throw new EntNotFoundError(id);
    return item;
  }
  async listComplianceReports(schoolId: string, filters?: Record<string, unknown>): Promise<GenericRecord[]> {
    return this.repo.findAllComplianceReports(schoolId, filters);
  }
  async createComplianceReport(schoolId: string, data: Record<string, unknown>): Promise<GenericRecord> {
    return this.repo.createComplianceReport(schoolId, data);
  }
  async updateComplianceReport(schoolId: string, id: string, data: Record<string, unknown>): Promise<GenericRecord> {
