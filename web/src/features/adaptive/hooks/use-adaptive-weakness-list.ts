'use client';

import { useState, useEffect, useCallback } from 'react';
import { AdaptiveWeaknessService } from '../services/adaptive-weakness.service';
import { createClient } from '@/lib/supabase/client';
import type { WeaknessDetection } from '@educi/types';

export const useAdaptiveWeaknessList = (schoolId: string) => {
  const [items, setItems] = useState<WeaknessDetection[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const fetchItems = useCallback(async () => {
    try {
      setLoading(true);
      const supabase = createClient();
      const service = new AdaptiveWeaknessService(supabase);
      const data = await service.listWeaknessDetections(schoolId);
      setItems(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erreur inconnue');
    } finally {
      setLoading(false);
    }
  }, [schoolId]);

  useEffect(() => { fetchItems(); }, [fetchItems]);

  return { items, loading, error, refresh: fetchItems };
};
