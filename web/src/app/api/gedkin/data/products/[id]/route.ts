...
import { createClient } from '@/utils/supabase/server';
  ...
  type: z.enum(['DATASET', 'API', 'FEED', 'REPORT', 'DASHBOARD', 'MODEL']).optional(),
...
export async function GET(
  ...
  try {
    ...
    const { data: record, error } = await supabase
      .from('gedkin_products')
      .select('*')
      .eq('id', id)
      .is('deleted_at', null)
      .single();

    if (error || !record) return NextResponse.json({ error: 'Produit introuvable' }, { status: 404 });
...
export async function PUT(
  ...
  try {
    ...
    if (data.domain_id !== undefined) updateData.domain_id = data.domain_id;
