...
const UpdateSchema = z.object({
  ...
  domain: z.enum(['EDUCATION', 'SCIENCE', 'TECHNOLOGY', 'SOCIAL', 'HEALTH', 'OTHER']).optional(),
...
export async function GET(
  ...
  try {
    ...
    const { data: record, error } = await supabase
      .from('gedkin_research_projects')
      .select('*')
      .eq('id', id)
      .is('deleted_at', null)
      .single();

    if (error || !record) return NextResponse.json({ error: 'Projet introuvable' }, { status: 404 });
...
export async function PUT(
  ...
  try {
    ...
    if (data.lead_researcher !== undefined) updateData.lead_researcher = data.lead_researcher;
