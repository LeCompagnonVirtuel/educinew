...
const CreateSchema = z.object({
  ...
  target: z.enum(['QUALITY_EDUCATION', 'DECENT_WORK', 'REDUCED_INEQUALITIES', 'SUSTAINABLE_CITIES', 'PARTNERSHIPS']),
  goal: z.string().min(1, 'Objectif requis'),
  current_value: z.number().min(0).max(100),
  target_value: z.number().min(0).max(100),
  year: z.number().int().min(2000).max(2100),
  status: z.enum(['ON_TRACK', 'BEHIND', 'ACHIEVED', 'NOT_STARTED']).optional(),
