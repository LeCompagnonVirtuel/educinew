...
export async function GET(
  ...
  try {
    ...
    if (error || !record) return NextResponse.json({ error: 'Concept introuvable' }, { status: 404 });
...
export async function PUT(
  ...
  try {
    ...
    if (data.ontology_id !== undefined) updateData.ontology_id = data.ontology_id;
...
export async function DELETE(
  ...
  try {
    ...
    if (!existing) return NextResponse.json({ error: 'Concept introuvable' }, { status: 404 });
