...
const CreateSchema = z.object({
  ...
  ontology_id: z.string().uuid('ID ontologie invalide'),
  root_concept: z.string().uuid('ID concept racine invalide'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      .from('gedkin_taxonomies')
      .select('*', { count: 'exact' })
      .eq('school_id', schoolId)
      .is('deleted_at', null);

    if (filters.ontology_id) query = query.eq('ontology_id', filters.ontology_id);
