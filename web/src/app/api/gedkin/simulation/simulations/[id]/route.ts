...
const UpdateSchema = z.object({
  ...
  model_type: z.enum(['SYSTEM_DYNAMICS', 'AGENT_BASED', 'DISCRETE_EVENT', 'MONTE_CARLO', 'FALL']).optional(),
...
export async function GET(
  ...
  try {
    ...
    const { data: record, error } = await supabase
      .from('gedkin_simulation_models')
      .select('*')
      .eq('id', id)
      .is('deleted_at', null)
      .single();

    if (error || !record) return NextResponse.json({ error: 'Simulation introuvable' }, { status: 404 });
...
export async function DELETE(
  ...
  try {
    ...
    if (!existing) return NextResponse.json({ error: 'Simulation introuvable' }, { status: 404 });
