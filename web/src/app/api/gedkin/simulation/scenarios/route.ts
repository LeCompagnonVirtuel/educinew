...
const CreateSchema = z.object({
  ...
  simulation_id: z.string().uuid('ID simulation invalide'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('gedkin_scenarios').select('*', { count: 'exact' }).eq('school_id', schoolId).is('deleted_at', null);
    if (filters.simulation_id) query = query.eq('simulation_id', filters.simulation_id);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('gedkin_scenarios').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      simulation_id: data.simulation_id, parameters: data.parameters,
      scenario_type: data.scenario_type || null, status: data.status || 'DRAFT',
      metadata: data.metadata || {}, created_by: user.id,
    }).select().single();
