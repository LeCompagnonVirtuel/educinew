...
const CreateSchema = z.object({
  scenario_id: z.string().uuid('ID scenario invalide'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('gedkin_runs').select('*', { count: 'exact' }).eq('school_id', schoolId).is('deleted_at', null);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('gedkin_runs').insert({
      school_id: schoolId, scenario_id: data.scenario_id, parameters: data.parameters || {},
      status: data.status || 'QUEUED', metadata: data.metadata || {}, created_by: user.id,
    }).select().single();
