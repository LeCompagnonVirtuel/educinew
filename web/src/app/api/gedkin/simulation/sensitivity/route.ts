...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('gedkin_sensitivity').select('*', { count: 'exact' }).eq('school_id', schoolId).is('deleted_at', null);
    if (filters.simulation_id) query = query.eq('simulation_id', filters.simulation_id);
    if (filters.parameter) query = query.eq('parameter', filters.parameter);
