...
const UpdateSchema = z.object({
  ...
  category: z.enum(['ACADEMIC', 'FINANCIAL', 'OPERATIONAL', 'HR', 'TECHNOLOGY', 'SOCIAL']).optional(),
  scope: z.enum(['SCHOOL', 'DEPARTMENT', 'CLASS', 'ALL']).optional(),
...
export async function GET(
  ...
  try {
    ...
    const { data: record, error } = await supabase
      .from('gedkin_policies')
      .select('*')
      .eq('id', id)
      .is('deleted_at', null)
      .single();

    if (error || !record) return NextResponse.json({ error: 'Politique introuvable' }, { status: 404 });
