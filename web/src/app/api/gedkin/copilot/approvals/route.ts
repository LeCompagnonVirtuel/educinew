...
const CreateSchema = z.object({
  ...
  action_type: z.enum(['DATA_ACCESS', 'CONFIG_CHANGE', 'USER_ACTION', 'EXPORT']),
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('gedkin_approvals').select('*', { count: 'exact' }).eq('school_id', schoolId).is('deleted_at', null);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('gedkin_approvals').insert({
      school_id: schoolId, user_id: user.id, title: data.title,
      description: data.description || null, action_type: data.action_type,
      action_config: data.action_config, status: data.status || 'PENDING',
