...
const ListResultsSchema = z.object({
  ...
  simulation_type: z.string().optional(),
});
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validation = ListResultsSchema.safeParse(params);
    ...
    let query = supabase
      .from('cyber_twin_results')
      .select('*', { count: 'exact' })
      .eq('school_id', schoolId);

    if (filters.twin_id) query = query.eq('twin_id', filters.twin_id);
    if (filters.simulation_type) query = query.eq('simulation_type', filters.simulation_type);
