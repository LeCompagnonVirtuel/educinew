...
const UpdateTwinSchema = z.object({
  ...
  type: z.enum(['NETWORK', 'APPLICATION', 'INFRASTRUCTURE', 'FULL_STACK']).optional(),
...
export async function GET(
  ...
  try {
    ...
    if (error || !twin) return NextResponse.json({ error: 'Jumeau numÃƒÂ©rique introuvable' }, { status: 404 });
...
export async function PUT(
  ...
  try {
    ...
    const validation = UpdateTwinSchema.safeParse(body);
...
export async function DELETE(
  ...
  try {
    ...
    if (!existing) return NextResponse.json({ error: 'Jumeau numÃƒÂ©rique introuvable' }, { status: 404 });
    if (existing.school_id !== profile?.school_id) {
      return NextResponse.json({ error: 'Non autorisÃƒÂ©' }, { status: 403 });
    }

    const { error } = await supabase.from('cyber_twins').delete().eq('id', id);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
