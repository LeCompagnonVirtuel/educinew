...
export async function PUT(
  ...
  try {
    ...
    if (data.backup_type !== undefined) updateData.backup_type = data.backup_type;
    if (data.targets !== undefined) updateData.targets = data.targets;
    if (data.encryption_enabled !== undefined) updateData.encryption_enabled = data.encryption_enabled;
...
export async function DELETE(
  ...
  try {
    ...
    const { error } = await supabase.from('backup_policies').delete().eq('id', id);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
