...
const UpdatePolicySchema = z.object({
  ...
  type: z.enum(['RBAC', 'ABAC', 'ACL', 'TIME_BASED', 'LOCATION_BASED']).optional(),
  statements: z.array(z.record(z.unknown())).optional(),
...
export async function PUT(
  ...
  try {
    ...
    if (data.statements !== undefined) updateData.statements = data.statements;
...
export async function DELETE(
  ...
  try {
    ...
    const { error } = await supabase.from('iam_policies').delete().eq('id', id);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
