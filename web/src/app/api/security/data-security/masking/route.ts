...
const ListMaskingRulesSchema = z.object({
...
const CreateMaskingRuleSchema = z.object({
  ...
  type: z.enum(['FULL', 'PARTIAL', 'HASH', 'TOKENIZE', 'SHIFT']),
  fields: z.array(z.string()).min(1, 'Champs requis'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validation = ListMaskingRulesSchema.safeParse(params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = CreateMaskingRuleSchema.safeParse(body);
    if (!validation.success) {
