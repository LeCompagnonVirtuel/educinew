...
export async function GET(
  ...
  try {
    ...
    if (error || !rule) return NextResponse.json({ error: 'RÃƒÂ¨gle introuvable' }, { status: 404 });
...
export async function DELETE(
  ...
  try {
    ...
    if (!existing) return NextResponse.json({ error: 'RÃƒÂ¨gle introuvable' }, { status: 404 });
    if (existing.school_id !== profile?.school_id) {
      return NextResponse.json({ error: 'Non autorisÃƒÂ©' }, { status: 403 });
    }

    const { error } = await supabase.from('siem_rules').delete().eq('id', id);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
