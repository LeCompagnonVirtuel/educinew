...
const CreateCorrelationSchema = z.object({
  name: z.string().min(1, 'Nom requis'),
  description: z.string().optional(),
  rule_ids: z.array(z.string().uuid()).min(1, 'Au moins une r̬gle requise'),
  logic: z.enum(['AND', 'OR', 'SEQUENCE']),
  time_window_seconds: z.number().int().positive(),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validation = ListCorrelationsSchema.safeParse(params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = CreateCorrelationSchema.safeParse(body);
    if (!validation.success) {
