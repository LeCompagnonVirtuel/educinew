...
const UpdatePolicySchema = z.object({
  ...
  type: z.enum(['RATE_LIMIT', 'IP_ALLOWLIST', 'IP_DENYLIST', 'AUTH_REQUIRED', 'CORS', 'CUSTOM']).optional(),
...
export async function DELETE(
  ...
  try {
    ...
    const { error } = await supabase.from('app_security_api_policies').delete().eq('id', id);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
