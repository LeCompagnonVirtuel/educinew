...
import { createClient } from '@/utils/supabase/server';
  ...
  trigger_type: z.enum(['MANUAL', 'EVENT', 'SCHEDULED', 'WEBHOOK']).optional(),
...
export async function GET(
  ...
  try {
    ...
    if (error || !playbook) return NextResponse.json({ error: 'Playbook introuvable' }, { status: 404 });
...
export async function DELETE(
  ...
  try {
    ...
    if (!existing) return NextResponse.json({ error: 'Playbook introuvable' }, { status: 404 });
