...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur récupération Tour de financement: ${error.message}` }, { status: 500 });
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur création Tour de financement: ${error.message}` }, { status: 500 });
    }
