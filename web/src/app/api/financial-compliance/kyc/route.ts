...
const kycSchema = z.object({
  ...
  verification_status: z.string().optional().optional(),
...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur r̩cup̩ration KYC: ${error.message}` }, { status: 500 });
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur cr̩ation KYC: ${error.message}` }, { status: 500 });
    }
