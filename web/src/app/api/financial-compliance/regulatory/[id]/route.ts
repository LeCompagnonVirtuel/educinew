...
export async function GET(
  ...
  try {
    ...
    if (error || !data) {
      return NextResponse.json({ error: 'DÃƒÂ©claration rÃƒÂ©glementaire non trouvÃƒÂ©' }, { status: 404 });
    }
...
export async function PUT(
  ...
  try {
    ...
    const validation = regulatoryUpdateSchema.safeParse(body);
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur mise ÃƒÂ  jour DÃƒÂ©claration rÃƒÂ©glementaire: ${error.message}` }, { status: 500 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur suppression DÃƒÂ©claration rÃƒÂ©glementaire: ${error.message}` }, { status: 500 });
    }

    return NextResponse.json({ message: 'DÃƒÂ©claration rÃƒÂ©glementaire supprimÃƒÂ© avec succÃƒÂ¨s' });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Erreur serveur' },
      { status: 500 }
