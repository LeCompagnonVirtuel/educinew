...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur récupération Déclaration réglementaire: ${error.message}` }, { status: 500 });
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur création Déclaration réglementaire: ${error.message}` }, { status: 500 });
    }
