...
const regulatorySchema = z.object({
  ...
  regulatory_body: z.string().optional().optional(),
...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur r̩cup̩ration D̩claration r̩glementaire: ${error.message}` }, { status: 500 });
    }
...
export async function POST(request: Request) {
  try {
    ...
    const validation = regulatorySchema.safeParse(body);
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur cr̩ation D̩claration r̩glementaire: ${error.message}` }, { status: 500 });
    }
