...
import { memberRoleSchema } from '@/features/messages/validators/schemas';
...
export async function GET(req: NextRequest, context: { params: { id: string } }) {
  try {
    ...
    logger.error('Error fetching conversation members', error);
...
export async function POST(req: NextRequest, context: { params: { id: string } }) {
  try {
    ...
    const validation = memberRoleSchema.safeParse({ ...body, conversationId: id });
    ...
    logger.error('Error adding conversation member', error);
    return NextResponse.json({ error: 'Erreur interne' }, { status: 500 });
  }
}
