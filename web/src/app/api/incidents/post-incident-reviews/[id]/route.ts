...
export const GET = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (error || !record) return Response.json({ error: 'Revue post-incident introuvable' }, { status: 404 });
...
export const PUT = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (!['SURVEILLANT', 'ADMIN', 'SUPER_ADMIN', 'DIRECTEUR'].includes(profile?.role)) {
...
export const DELETE = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  const { error } = await supabase.from('post_incident_reviews').delete().eq('id', id).eq('school_id', ctx.schoolId);
