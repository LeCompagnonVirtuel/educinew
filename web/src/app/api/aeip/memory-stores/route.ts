...
import { MemoryStoreCreateSchema } from '@/features/aeip/validators/digital-brain-validators';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await service.findAll(schoolId, { page, limit, brainId, memoryType });

    return NextResponse.json(result);
  } catch (error) {
    logger.error('Erreur lors de la liste des magasins de memoire:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = MemoryStoreCreateSchema.safeParse(body);
    ...
    logger.error('Erreur lors de la creation du magasin de memoire:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
