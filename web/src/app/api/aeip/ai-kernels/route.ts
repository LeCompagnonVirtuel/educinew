...
import { AIKernelCreateSchema } from '@/features/aeip/validators/ai-os-validators';
...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error('Erreur lors de la liste des noyaux IA:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = AIKernelCreateSchema.safeParse(body);
    ...
    logger.error('Erreur lors de la cr̩ation du noyau IA:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
