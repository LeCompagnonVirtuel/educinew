...
import { DigitalBrainUpdateSchema } from '@/features/aeip/validators/digital-brain-validators';
...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!result) return NextResponse.json({ error: 'Cerveau digital introuvable' }, { status: 404 });

    return NextResponse.json(result);
  } catch (error) {
    logger.error('Erreur lors de la recuperation du cerveau digital:', error);
...
export async function PUT(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validation = DigitalBrainUpdateSchema.safeParse(body);
    ...
    logger.error('Erreur lors de la mise a jour du cerveau digital:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    logger.error('Erreur lors de la suppression du cerveau digital:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}