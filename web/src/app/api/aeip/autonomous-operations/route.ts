...
import { AutonomousOperationCreateSchema } from '@/features/aeip/validators/autonomous-ops-validators';
...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error('Erreur lors de la liste des op̩rations autonomes:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = AutonomousOperationCreateSchema.safeParse(body);
    ...
    logger.error('Erreur lors de la cr̩ation de l\'op̩ration autonome:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
