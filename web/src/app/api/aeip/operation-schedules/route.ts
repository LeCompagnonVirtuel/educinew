...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error('Erreur lors de la liste des planifications d\'opération:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    logger.error('Erreur lors de la création de la planification d\'opération:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
