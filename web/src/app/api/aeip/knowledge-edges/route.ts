...
import { KnowledgeEdgeCreateSchema } from '@/features/aeip/validators/digital-brain-validators';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await service.findAll(schoolId, { page, limit, sourceId, targetId });

    return NextResponse.json(result);
  } catch (error) {
    logger.error('Erreur lors de la liste des edges de connaissance:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = KnowledgeEdgeCreateSchema.safeParse(body);
    ...
    logger.error('Erreur lors de la creation de l\'edge de connaissance:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
