...
import { AgentCollaborationCreateSchema } from '@/features/aeip/validators/multi-agent-validators';
...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error('Erreur lors de la liste des collaborations d\'agent:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = AgentCollaborationCreateSchema.safeParse(body);
    ...
    logger.error('Erreur lors de la cr̩ation de la collaboration d\'agent:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
