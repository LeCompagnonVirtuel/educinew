...
import { DecisionFeedbackUpdateSchema } from '@/features/aeip/validators/ai-decision-validators';
...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!result) return NextResponse.json({ error: 'Retour de decision introuvable' }, { status: 404 });

    return NextResponse.json(result);
  } catch (error) {
    logger.error('Erreur lors de la recuperation du retour de decision:', error);
...
export async function PUT(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validation = DecisionFeedbackUpdateSchema.safeParse(body);
    ...
    logger.error('Erreur lors de la mise a jour du retour de decision:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    logger.error('Erreur lors de la suppression du retour de decision:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}