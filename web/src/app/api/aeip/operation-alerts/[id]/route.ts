...
import { createClient } from '@/utils/supabase/server';
  try {
    ...
    if (!result) return NextResponse.json({ error: 'Alerte d\'opÃ©ration introuvable' }, { status: 404 });

    return NextResponse.json(result);
  } catch (error) {
    logger.error('Erreur lors de la rÃ©cupÃ©ration de l\'alerte d\'opÃ©ration:', error);
...
export async function PUT(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validation = OperationAlertUpdateSchema.safeParse(body);
    ...
    logger.error('Erreur lors de la mise Ã  jour de l\'alerte d\'opÃ©ration:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    logger.error('Erreur lors de la suppression de l\'alerte d\'opÃ©ration:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}