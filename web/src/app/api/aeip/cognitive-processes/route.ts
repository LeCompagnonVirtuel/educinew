...
import { CognitiveProcessCreateSchema } from '@/features/aeip/validators/digital-brain-validators';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const service = new AeipCognitiveProcessService(supabase);
    const result = await service.findAll(schoolId, { page, limit, brainId, processType, status });

    return NextResponse.json(result);
  } catch (error) {
    logger.error('Erreur lors de la liste des processus cognitifs:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = CognitiveProcessCreateSchema.safeParse(body);
    ...
    logger.error('Erreur lors de la creation du processus cognitif:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
