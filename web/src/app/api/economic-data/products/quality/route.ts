...
const qualitySchema = z.object({
  ...
  measured_at: z.string().optional().optional(),
});
...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur r̩cup̩ration Qualit̩ de donn̩es: ${error.message}` }, { status: 500 });
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur cr̩ation Qualit̩ de donn̩es: ${error.message}` }, { status: 500 });
    }
