...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const service = new EntSurveyResponseService(supabase);
    const data = await service.getSurveyResponse(schoolId, parentId, params.id);
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const service = new EntSurveyResponseService(supabase);
    const data = await service.updateSurveyResponse(body.schoolId, body.parentId, params.id, body);
    return NextResponse.json({ data });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Erreur inconnue';
    return NextResponse.json({ error: message }, { status: 500 });
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!parentId) return NextResponse.json({ error: 'parentId required' }, { status: 400 });
    const service = new EntSurveyResponseService(supabase);
    const data = await service.deleteSurveyResponse(schoolId, parentId, params.id);
    return NextResponse.json({ data });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Erreur inconnue';
    return NextResponse.json({ error: message }, { status: 500 });
