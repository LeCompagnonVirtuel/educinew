...
export const GET = withSupabase({ auth: 'user' }, async (req: NextRequest, ctx: any) => {
  try {
    ...
    const data = await service.getSchoolGrade(schoolId, parentId, id!);
...
export const PUT = withSupabase({ auth: 'user' }, async (req: NextRequest, ctx: any) => {
  try {
    ...
    const serviceClient = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!);
    const service = new EntSchoolGradeService(serviceClient);
    const data = await service.updateSchoolGrade(schoolId, parentId, id!, body);
    return NextResponse.json({ data });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Erreur inconnue';
    return NextResponse.json({ error: message }, { status: 500 });
...
export const DELETE = withSupabase({ auth: 'user' }, async (req: NextRequest, ctx: any) => {
  try {
    ...
    const serviceClient = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!);
    const service = new EntSchoolGradeService(serviceClient);
    const data = await service.deleteSchoolGrade(schoolId, parentId, id!);
    return NextResponse.json({ data });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Erreur inconnue';
    return NextResponse.json({ error: message }, { status: 500 });
