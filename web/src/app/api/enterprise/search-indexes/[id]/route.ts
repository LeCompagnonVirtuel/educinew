...
import { EntSearchIndexService } from '@/features/enterprise/services/ent-search-indexes.service';
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const data = await service.getSearchIndex(schoolId, params.id);
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const service = new EntSearchIndexService(supabase);
    const data = await service.updateSearchIndex(body.schoolId, params.id, body);
    return NextResponse.json({ data });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Erreur inconnue';
    return NextResponse.json({ error: message }, { status: 500 });
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!schoolId) return NextResponse.json({ error: 'schoolId required' }, { status: 400 });
    const service = new EntSearchIndexService(supabase);
    const data = await service.deleteSearchIndex(schoolId, params.id);
    return NextResponse.json({ data });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Erreur inconnue';
    return NextResponse.json({ error: message }, { status: 500 });
