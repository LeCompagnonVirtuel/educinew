...
import { EntDeploymentApprovalService } from '@/features/enterprise/services/ent-deployments-approvals.service';
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const data = await service.getDeploymentApproval(schoolId, parentId, params.id);
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const service = new EntDeploymentApprovalService(supabase);
    const data = await service.updateDeploymentApproval(body.schoolId, body.parentId, params.id, body);
    return NextResponse.json({ data });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Erreur inconnue';
    return NextResponse.json({ error: message }, { status: 500 });
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!parentId) return NextResponse.json({ error: 'parentId required' }, { status: 400 });
    const service = new EntDeploymentApprovalService(supabase);
    const data = await service.deleteDeploymentApproval(schoolId, parentId, params.id);
    return NextResponse.json({ data });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Erreur inconnue';
    return NextResponse.json({ error: message }, { status: 500 });
