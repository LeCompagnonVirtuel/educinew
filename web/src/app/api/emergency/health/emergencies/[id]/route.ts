...
export const GET = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (error || !record) return Response.json({ error: 'Urgence introuvable' }, { status: 404 });
...
export const DELETE = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  const { error } = await supabase.from('emergencies').delete().eq('id', id).eq('school_id', ctx.schoolId);
