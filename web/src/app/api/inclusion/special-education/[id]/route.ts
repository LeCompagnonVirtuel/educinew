...
export const GET = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  const { data: record, error } = await supabase
    .from('special_education')
    .select('*, student:students(id, first_name, last_name, matricule)')
    .eq('id', id).eq('school_id', ctx.schoolId)
    .single();

  if (error || !record) return Response.json({ error: 'Éducation spécialisée introuvable' }, { status: 404 });
...
export const PUT = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (!['ADMIN', 'SUPER_ADMIN', 'DIRECTEUR', 'ENSEIGNANT'].includes(profile?.role)) {
...
export const DELETE = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  const { error } = await supabase.from('special_education').delete().eq('id', id).eq('school_id', ctx.schoolId);
