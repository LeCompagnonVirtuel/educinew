...
export async function GET(
  ...
  try {
    ...
    if (error || !data) {
      return NextResponse.json({ error: 'Don non trouvÃƒÂ©' }, { status: 404 });
    }
...
export async function PUT(
  ...
  try {
    ...
    const validation = donationsUpdateSchema.safeParse(body);
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur mise ÃƒÂ  jour Don: ${error.message}` }, { status: 500 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ message: 'Don supprimÃƒÂ© avec succÃƒÂ¨s' });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Erreur serveur' },
      { status: 500 }
