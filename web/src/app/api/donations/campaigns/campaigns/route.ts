...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur r̩cup̩ration Campagne de dons: ${error.message}` }, { status: 500 });
    }
...
export async function POST(request: Request) {
  try {
    ...
    const validation = campaignsSchema.safeParse(body);
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur cr̩ation Campagne de dons: ${error.message}` }, { status: 500 });
    }
