...
export async function GET(
  ...
  try {
    ...
    const data = await service.getExamCandidate(profile.school_id, id);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET exam-candidate/[id]', error);
...
export async function PUT(
  ...
  try {
    ...
    const service = new GovExamCandidateService(supabase);
    const data = await service.updateExamCandidate(profile.school_id, id, validation.data);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur PUT exam-candidate/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const service = new GovExamCandidateService(supabase);
    await service.deleteExamCandidate(profile.school_id, id);
    return NextResponse.json({ success: true });
  } catch (error) {
    logger.error('Erreur DELETE exam-candidate/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}