...
import { examCandidateCreateSchema, examCandidateUpdateSchema } from '@/features/gov/validators/gov-exams-inspection-accreditation';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = new GovExamCandidateService(supabase);
    const { searchParams } = new URL(request.url);
    const filters: Record<string, unknown> = {};
    searchParams.forEach((value, key) => { filters[key] = value; });

    const data = await service.listExamCandidates(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET exam-candidate', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = examCandidateCreateSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: 'Données invalides', details: validation.error.flatten().fieldErrors }, { status: 400 });
    }

    const service = new GovExamCandidateService(supabase);
    const data = await service.createExamCandidate(profile.school_id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST exam-candidate', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}