...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = new GovCountryService(supabase);
    const { searchParams } = new URL(request.url);
    const filters: Record<string, unknown> = {};
    searchParams.forEach((value, key) => { filters[key] = value; });

    const data = await service.listCountries(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET country', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const service = new GovCountryService(supabase);
    const data = await service.createCountry(profile.school_id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST country', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}