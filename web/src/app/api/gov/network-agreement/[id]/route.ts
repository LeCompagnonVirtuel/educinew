...
export async function GET(
  ...
  try {
    ...
    const data = await service.getNetworkAgreement(profile.school_id, id);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET network-agreement/[id]', error);
...
export async function PUT(
  ...
  try {
    ...
    const service = new GovNetworkAgreementService(supabase);
    const data = await service.updateNetworkAgreement(profile.school_id, id, validation.data);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur PUT network-agreement/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const service = new GovNetworkAgreementService(supabase);
    await service.deleteNetworkAgreement(profile.school_id, id);
    return NextResponse.json({ success: true });
  } catch (error) {
    logger.error('Erreur DELETE network-agreement/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}