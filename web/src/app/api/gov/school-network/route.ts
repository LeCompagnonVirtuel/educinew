...
import { schoolNetworkCreateSchema, schoolNetworkUpdateSchema } from '@/features/gov/validators/gov-campus-network';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listSchoolNetworks(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET school-network', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = schoolNetworkCreateSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: 'Données invalides', details: validation.error.flatten().fieldErrors }, { status: 400 });
    }

    const service = new GovSchoolNetworkService(supabase);
    const data = await service.createSchoolNetwork(profile.school_id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST school-network', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}