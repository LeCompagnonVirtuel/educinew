...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listInternationalPartnerships(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET international-partnership', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const service = new GovInternationalPartnershipService(supabase);
    const data = await service.createInternationalPartnership(profile.school_id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST international-partnership', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}