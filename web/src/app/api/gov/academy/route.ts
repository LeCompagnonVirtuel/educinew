...
import { academyCreateSchema, academyUpdateSchema } from '@/features/gov/validators/gov-ministry-region';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listAcademies(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET academy', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = academyCreateSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: 'Données invalides', details: validation.error.flatten().fieldErrors }, { status: 400 });
    }

    const service = new GovAcademyService(supabase);
    const data = await service.createAcademy(profile.school_id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST academy', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
