...
import { diplomaUpdateSchema } from '@/features/gov/validators/gov-exams-inspection-accreditation';
...
export async function GET(
  ...
  try {
    ...
    const data = await service.getDiploma(profile.school_id, id);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET diploma/[id]', error);
...
export async function PUT(
  ...
  try {
    ...
    const validation = diplomaUpdateSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: 'Données invalides', details: validation.error.flatten().fieldErrors }, { status: 400 });
    }

    const service = new GovDiplomaService(supabase);
    const data = await service.updateDiploma(profile.school_id, id, validation.data);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur PUT diploma/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const service = new GovDiplomaService(supabase);
    await service.deleteDiploma(profile.school_id, id);
    return NextResponse.json({ success: true });
  } catch (error) {
    logger.error('Erreur DELETE diploma/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}