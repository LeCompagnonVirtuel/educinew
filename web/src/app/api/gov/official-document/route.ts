...
import { officialDocumentCreateSchema, officialDocumentUpdateSchema } from '@/features/gov/validators/gov-ministry-region';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listOfficialDocuments(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET official-document', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = officialDocumentCreateSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: 'Données invalides', details: validation.error.flatten().fieldErrors }, { status: 400 });
    }

    const service = new GovOfficialDocumentService(supabase);
    const data = await service.createOfficialDocument(profile.school_id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST official-document', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
