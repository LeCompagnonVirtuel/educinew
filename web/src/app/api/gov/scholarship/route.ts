...
import { scholarshipCreateSchema, scholarshipUpdateSchema } from '@/features/gov/validators/gov-analytics-funding-identity';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listScholarships(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET scholarship', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = scholarshipCreateSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: 'Données invalides', details: validation.error.flatten().fieldErrors }, { status: 400 });
    }

    const service = new GovScholarshipService(supabase);
    const data = await service.createScholarship(profile.school_id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST scholarship', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}