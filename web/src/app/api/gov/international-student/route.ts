...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listInternationalStudents(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET international-student', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const service = new GovInternationalStudentService(supabase);
    const data = await service.createInternationalStudent(profile.school_id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST international-student', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}