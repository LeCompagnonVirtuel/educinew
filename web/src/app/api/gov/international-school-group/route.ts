...
import { internationalSchoolGroupCreateSchema, internationalSchoolGroupUpdateSchema } from '@/features/gov/validators/gov-campus-network';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listInternationalSchoolGroups(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET international-school-group', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = internationalSchoolGroupCreateSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: 'Donn̩es invalides', details: validation.error.flatten().fieldErrors }, { status: 400 });
    }

    const service = new GovInternationalSchoolGroupService(supabase);
    const data = await service.createInternationalSchoolGroup(profile.school_id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST international-school-group', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}