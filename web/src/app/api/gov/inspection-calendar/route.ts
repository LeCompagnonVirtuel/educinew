...
import { inspectionCalendarCreateSchema, inspectionCalendarUpdateSchema } from '@/features/gov/validators/gov-exams-inspection-accreditation';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listInspectionCalendars(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET inspection-calendar', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = inspectionCalendarCreateSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: 'Données invalides', details: validation.error.flatten().fieldErrors }, { status: 400 });
    }

    const service = new GovInspectionCalendarService(supabase);
    const data = await service.createInspectionCalendar(profile.school_id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST inspection-calendar', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}