...
import { examCenterCreateSchema, examCenterUpdateSchema } from '@/features/gov/validators/gov-exams-inspection-accreditation';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listExamCenters(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET exam-center', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = examCenterCreateSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: 'Données invalides', details: validation.error.flatten().fieldErrors }, { status: 400 });
    }

    const service = new GovExamCenterService(supabase);
    const data = await service.createExamCenter(profile.school_id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST exam-center', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}