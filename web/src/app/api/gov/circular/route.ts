...
import { circularCreateSchema, circularUpdateSchema } from '@/features/gov/validators/gov-ministry-region';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listCirculars(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET circular', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = circularCreateSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: 'Données invalides', details: validation.error.flatten().fieldErrors }, { status: 400 });
    }

    const service = new GovCircularService(supabase);
    const data = await service.createCircular(profile.school_id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST circular', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
