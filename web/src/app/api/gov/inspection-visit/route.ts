...
import { inspectionVisitCreateSchema, inspectionVisitUpdateSchema } from '@/features/gov/validators/gov-ministry-region';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listInspectionVisits(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET inspection-visit', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const service = new GovInspectionVisitService(supabase);
    const data = await service.createInspectionVisit(profile.school_id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST inspection-visit', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
