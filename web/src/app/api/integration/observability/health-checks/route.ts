...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listHealthChecks(params.schoolId || '', params.userId || '', params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createIntegrationRepository(supabase);
    const service = createHealthCheckService(repo);
    const data = await service.createHealthCheck(body.schoolId, body.userId, body);
