...
import { createWebhookTemplateService } from '@/features/integration/services/webhook-template.service';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = createWebhookTemplateService(repo);
    const { searchParams } = new URL(request.url);
    const params = Object.fromEntries(searchParams);
    const data = await service.listWebhookTemplates(params.schoolId || '', params.userId || '', params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createIntegrationRepository(supabase);
    const service = createWebhookTemplateService(repo);
    const data = await service.createWebhookTemplate(body.schoolId, body.userId, body);
