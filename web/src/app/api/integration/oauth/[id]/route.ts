...
import { createOAuthService } from '@/features/integration/services/oauth.service';
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const data = await service.getOAuthConfig(schoolId, userId, params.id);
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const repo = createIntegrationRepository(supabase);
    const service = createOAuthService(repo);
    const data = await service.updateOAuthConfig(body.schoolId, body.userId, params.id, body);
    return NextResponse.json({ data });
  } catch (error: unknown) {
    const err = error instanceof Error ? error : new Error(String(error));
    return NextResponse.json({ error: err.message }, { status: (error as { statusCode?: number }).statusCode || 500 });
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const repo = createIntegrationRepository(supabase);
    const service = createOAuthService(repo);
    await service.deleteOAuthConfig(schoolId, userId, params.id);
