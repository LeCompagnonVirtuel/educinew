...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listSubscriptions(params.schoolId || '', params.userId || '', params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createIntegrationRepository(supabase);
    const service = createSubscriptionService(repo);
    const data = await service.createSubscription(body.schoolId, body.userId, body);
