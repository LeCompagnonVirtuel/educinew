...
import { createApiVersionService } from '@/features/integration/services/api-version.service';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = createApiVersionService(repo);
    const { searchParams } = new URL(request.url);
    const params = Object.fromEntries(searchParams);
    const data = await service.listApiVersions(params.schoolId || '', params.userId || '', params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createIntegrationRepository(supabase);
    const service = createApiVersionService(repo);
    const data = await service.createApiVersion(body.schoolId, body.userId, body);
