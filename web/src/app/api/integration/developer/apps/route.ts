...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listDeveloperApps(params.schoolId || '', params.userId || '', params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createIntegrationRepository(supabase);
    const service = createDeveloperAppService(repo);
    const data = await service.createDeveloperApp(body.schoolId, body.userId, body);
