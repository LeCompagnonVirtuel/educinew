...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listMarketplaceItems(params.schoolId || '', params.userId || '', params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createIntegrationRepository(supabase);
    const service = createMarketplaceService(repo);
    const data = await service.createMarketplaceItem(body.schoolId, body.userId, body);
