...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listAutomations(params.schoolId || '', params.userId || '', { ...params, type: 'template' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createIntegrationRepository(supabase);
    const service = createAutomationService(repo);
    const data = await service.createAutomation(body.schoolId, body.userId, { ...body, isTemplate: true });
