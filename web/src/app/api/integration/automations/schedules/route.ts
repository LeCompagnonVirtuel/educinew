...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listAutomations(params.schoolId || '', params.userId || '', { ...params, hasSchedule: true });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createIntegrationRepository(supabase);
    const service = createAutomationService(repo);
    const data = await service.scheduleAutomation(body.schoolId, body.userId, body.automationId, body);
