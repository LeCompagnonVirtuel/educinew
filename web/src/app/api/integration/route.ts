...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listIntegrations(params.schoolId || '', params.userId || '', params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createIntegrationRepository(supabase);
    const service = createIntegrationService(repo);
    const data = await service.createIntegration(body.schoolId, body.userId, body);
