...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listKnowledgeBases(params.schoolId || '', params.userId || '', params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createIntegrationRepository(supabase);
    const service = createAiKnowledgeBaseService(repo);
    const data = await service.createKnowledgeBase(body.schoolId, body.userId, body);
