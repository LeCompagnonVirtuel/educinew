...
import { createLegalHoldService } from '@/features/documents/services/legal-hold.service';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = createLegalHoldService(repo);
    const { searchParams } = new URL(request.url);
    const params = Object.fromEntries(searchParams);
    const data = await service.listLegalHolds(params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createDocumentRepository(serviceRoleSupabase);
    const service = createLegalHoldService(repo);
    const data = await service.createLegalHold(body);
