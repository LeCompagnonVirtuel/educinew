...
import { createStorageService } from '@/features/documents/services/storage.service';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.getStorageQuota();
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const repo = createDocumentRepository(serviceRoleSupabase);
    const service = createStorageService(repo);
    const data = await service.updateStorageQuota(body);
    return NextResponse.json({ data });
  } catch (error: unknown) {
    const err = error instanceof Error ? error : new Error(String(error));
    return NextResponse.json({ error: err.message }, { status: (error as { statusCode?: number }).statusCode || 500 });
