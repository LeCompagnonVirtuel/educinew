...
import { createRetentionService } from '@/features/documents/services/retention.service';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = createRetentionService(repo);
    const { searchParams } = new URL(request.url);
    const params = Object.fromEntries(searchParams);
    const data = await service.listRetentionPolicies(params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createDocumentRepository(serviceRoleSupabase);
    const service = createRetentionService(repo);
    const data = await service.createRetentionPolicy(body);
