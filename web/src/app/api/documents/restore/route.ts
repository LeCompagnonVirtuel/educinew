...
import { createRestoreService } from '@/features/documents/services/restore.service';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const service = createRestoreService(repo);
    const { searchParams } = new URL(request.url);
    const params = Object.fromEntries(searchParams);
    const data = await service.listRestores(params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createDocumentRepository(serviceRoleSupabase);
    const service = createRestoreService(repo);
    const data = await service.createRestore(body);
