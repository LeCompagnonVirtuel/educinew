...
import { createFolderService } from '@/features/documents/services/folder.service';
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const data = await service.getFolderPermissions(id);
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const repo = createDocumentRepository(serviceRoleSupabase);
    const service = createFolderService(repo);
    const data = await service.setFolderPermission(id, body);
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const repo = createDocumentRepository(serviceRoleSupabase);
    const service = createFolderService(repo);
    const data = await service.removeFolderPermission(id, body);
    return NextResponse.json({ data });
  } catch (error: unknown) {
    const err = error instanceof Error ? error : new Error(String(error));
    return NextResponse.json({ error: err.message }, { status: (error as { statusCode?: number }).statusCode || 500 });
