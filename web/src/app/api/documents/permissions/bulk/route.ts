...
import { createPermissionService } from '@/features/documents/services/permission.service';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = await service.bulkCreatePermissions(body);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const repo = createDocumentRepository(serviceRoleSupabase);
    const service = createPermissionService(repo);
    const data = await service.bulkDeletePermissions(body);
