...
import { createBackupService } from '@/features/documents/services/backup.service';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.getBackupSchedule();
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const repo = createDocumentRepository(serviceRoleSupabase);
    const service = createBackupService(repo);
    const data = await service.updateBackupSchedule(body);
    return NextResponse.json({ data });
  } catch (error: unknown) {
    const err = error instanceof Error ? error : new Error(String(error));
    return NextResponse.json({ error: err.message }, { status: (error as { statusCode?: number }).statusCode || 500 });
