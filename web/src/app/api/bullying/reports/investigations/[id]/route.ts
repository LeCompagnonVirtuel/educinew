...
export const GET = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (error || !record) return Response.json({ error: 'Enquête introuvable' }, { status: 404 });
...
export const DELETE = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  const { error } = await supabase.from('investigations').delete().eq('id', id).eq('school_id', ctx.schoolId);
