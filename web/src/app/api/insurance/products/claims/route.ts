...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur r̩cup̩ration R̩clamation d\'assurance: ${error.message}` }, { status: 500 });
    }
...
export async function POST(request: Request) {
  try {
    ...
    const validation = claimsSchema.safeParse(body);
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur cr̩ation R̩clamation d\'assurance: ${error.message}` }, { status: 500 });
    }
