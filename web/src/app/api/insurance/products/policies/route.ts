...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur r̩cup̩ration Police d\'assurance: ${error.message}` }, { status: 500 });
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur cr̩ation Police d\'assurance: ${error.message}` }, { status: 500 });
    }
