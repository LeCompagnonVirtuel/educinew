...
import { competencyTestCreateSchema } from '@/features/assessment/validators/assessment-certification';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listCompetencyTests(schoolId, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Error fetching competency tests', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = competencyTestCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentCompetencyTestService(supabase);
    const data = await service.createCompetencyTest(schoolId, validation.data);
    return NextResponse.json(data, { status: 201 });
  } catch (error) {
    logger.error('Error creating competency test', error);
    return NextResponse.json({ error: 'Erreur interne' }, { status: 500 });
  }
}
