...
import { examAttemptCreateSchema } from '@/features/assessment/validators/assessment-core';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listExamAttempts(schoolId, filters);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = examAttemptCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentExamAttemptService(supabase);
    const data = await service.createExamAttempt(schoolId, validation.data);
