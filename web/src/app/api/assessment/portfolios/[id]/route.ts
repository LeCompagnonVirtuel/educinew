...
import { portfolioUpdateSchema } from '@/features/assessment/validators/assessment-certification';
...
export async function GET(req: NextRequest, context: { params: { id: string } }) {
  try {
    ...
    const data = await service.getPortfolio(schoolId, id);
...
export async function PUT(req: NextRequest, context: { params: { id: string } }) {
  try {
    ...
    const service = new AssessmentPortfolioService(supabase);
    const data = await service.updatePortfolio(schoolId, id, validation.data);
    return NextResponse.json(data);
  } catch (error) {
