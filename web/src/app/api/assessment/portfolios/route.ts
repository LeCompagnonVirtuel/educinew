...
import { portfolioCreateSchema } from '@/features/assessment/validators/assessment-certification';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listPortfolios(schoolId, filters);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = portfolioCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentPortfolioService(supabase);
    const data = await service.createPortfolio(schoolId, validation.data);
