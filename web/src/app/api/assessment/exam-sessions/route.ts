...
import { examSessionCreateSchema } from '@/features/assessment/validators/assessment-core';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listExamSessions(schoolId, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Error fetching exam sessions', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = examSessionCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentExamSessionService(supabase);
    const data = await service.createExamSession(schoolId, validation.data);
