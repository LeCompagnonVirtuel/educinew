...
import { transcriptGeneratorUpdateSchema } from '@/features/assessment/validators/assessment-certification';
...
export async function GET(req: NextRequest, context: { params: { id: string } }) {
  try {
    ...
    const data = await service.getTranscript(schoolId, id);
...
export async function PUT(req: NextRequest, context: { params: { id: string } }) {
  try {
    ...
    const validation = transcriptGeneratorUpdateSchema.safeParse(body);
    ...
    const service = new AssessmentTranscriptService(supabase);
    const data = await service.updateTranscript(schoolId, id, validation.data);
    return NextResponse.json(data);
  } catch (error) {
