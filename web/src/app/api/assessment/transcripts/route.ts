...
import { transcriptGeneratorCreateSchema } from '@/features/assessment/validators/assessment-certification';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listTranscripts(schoolId, filters);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = transcriptGeneratorCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentTranscriptService(supabase);
    const data = await service.createTranscript(schoolId, validation.data);
