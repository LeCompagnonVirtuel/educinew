...
import { proctoringAICreateSchema } from '@/features/assessment/validators/assessment-core';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listProctorings(schoolId, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Error fetching proctorings', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = proctoringAICreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentProctoringService(supabase);
    const data = await service.createProctoring(schoolId, validation.data);
    return NextResponse.json(data, { status: 201 });
  } catch (error) {
    logger.error('Error creating proctoring', error);
    return NextResponse.json({ error: 'Erreur interne' }, { status: 500 });
  }
}
