...
import { skillBadgeCreateSchema } from '@/features/assessment/validators/assessment-certification';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listSkillBadges(schoolId, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Error fetching skill badges', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = skillBadgeCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentSkillBadgeService(supabase);
    const data = await service.createSkillBadge(schoolId, validation.data);
    return NextResponse.json(data, { status: 201 });
  } catch (error) {
    logger.error('Error creating skill badge', error);
    return NextResponse.json({ error: 'Erreur interne' }, { status: 500 });
  }
}
