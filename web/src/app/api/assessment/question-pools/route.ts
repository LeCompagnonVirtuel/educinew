...
import { questionPoolCreateSchema } from '@/features/assessment/validators/assessment-core';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listQuestionPools(schoolId, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Error fetching question pools', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = questionPoolCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentQuestionPoolService(supabase);
    const data = await service.createQuestionPool(schoolId, validation.data);
    return NextResponse.json(data, { status: 201 });
  } catch (error) {
    logger.error('Error creating question pool', error);
    return NextResponse.json({ error: 'Erreur interne' }, { status: 500 });
  }
}
