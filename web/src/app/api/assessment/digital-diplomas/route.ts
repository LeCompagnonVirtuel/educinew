...
import { digitalDiplomaCreateSchema } from '@/features/assessment/validators/assessment-certification';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listDigitalDiplomas(schoolId, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Error fetching digital diplomas', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = digitalDiplomaCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentDigitalDiplomaService(supabase);
    const data = await service.createDigitalDiploma(schoolId, validation.data);
    return NextResponse.json(data, { status: 201 });
  } catch (error) {
    logger.error('Error creating digital diploma', error);
    return NextResponse.json({ error: 'Erreur interne' }, { status: 500 });
  }
}
