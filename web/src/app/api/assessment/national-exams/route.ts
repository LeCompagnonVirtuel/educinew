...
import { nationalExamCreateSchema } from '@/features/assessment/validators/assessment-certification';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listNationalExams(schoolId, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Error fetching national exams', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = nationalExamCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentNationalExamService(supabase);
    const data = await service.createNationalExam(schoolId, validation.data);
    return NextResponse.json(data, { status: 201 });
  } catch (error) {
    logger.error('Error creating national exam', error);
    return NextResponse.json({ error: 'Erreur interne' }, { status: 500 });
  }
}
