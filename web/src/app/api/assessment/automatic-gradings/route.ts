...
import { automaticGradingCreateSchema } from '@/features/assessment/validators/assessment-core';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listAutomaticGradings(schoolId, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Error fetching automatic gradings', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = automaticGradingCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentAutomaticGradingService(supabase);
    const data = await service.createAutomaticGrading(schoolId, validation.data);
    return NextResponse.json(data, { status: 201 });
  } catch (error) {
    logger.error('Error creating automatic grading', error);
    return NextResponse.json({ error: 'Erreur interne' }, { status: 500 });
  }
}
