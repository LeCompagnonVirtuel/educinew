...
import { schoolAccreditationCreateSchema } from '@/features/assessment/validators/assessment-modules';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listAccreditations(schoolId, filters);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = schoolAccreditationCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentAccreditationService(supabase);
    const data = await service.createAccreditation(schoolId, validation.data);
