...
import { researchProjectCreateSchema } from '@/features/assessment/validators/assessment-research-intl';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listResearchProjects(schoolId, filters);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = researchProjectCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentResearchProjectService(supabase);
    const data = await service.createResearchProject(schoolId, validation.data);
