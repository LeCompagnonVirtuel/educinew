...
import { researchProjectUpdateSchema } from '@/features/assessment/validators/assessment-research-intl';
...
export async function GET(req: NextRequest, context: { params: { id: string } }) {
  try {
    ...
    const data = await service.getResearchProject(schoolId, id);
    return NextResponse.json(data);
  } catch (error) {
    logger.error('Error fetching research project', error);
...
export async function PUT(req: NextRequest, context: { params: { id: string } }) {
  try {
    ...
    const validation = researchProjectUpdateSchema.safeParse(body);
    ...
    const service = new AssessmentResearchProjectService(supabase);
    const data = await service.updateResearchProject(schoolId, id, validation.data);
    return NextResponse.json(data);
  } catch (error) {
