...
import { adaptiveExamCreateSchema } from '@/features/assessment/validators/assessment-core';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listAdaptiveExams(schoolId, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Error fetching adaptive exams', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = adaptiveExamCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentAdaptiveExamService(supabase);
    const data = await service.createAdaptiveExam(schoolId, validation.data);
    return NextResponse.json(data, { status: 201 });
  } catch (error) {
    logger.error('Error creating adaptive exam', error);
    return NextResponse.json({ error: 'Erreur interne' }, { status: 500 });
  }
}
