...
import { certificateCreateSchema } from '@/features/assessment/validators/assessment-certification';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listCertificates(schoolId, filters);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = certificateCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentCertificateService(supabase);
    const data = await service.createCertificate(schoolId, validation.data);
