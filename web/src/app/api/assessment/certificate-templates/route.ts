...
import { certificateTemplateCreateSchema } from '@/features/assessment/validators/assessment-certification';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listCertificateTemplates(schoolId, filters);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = certificateTemplateCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new AssessmentCertificateTemplateService(supabase);
    const data = await service.createCertificateTemplate(schoolId, validation.data);
