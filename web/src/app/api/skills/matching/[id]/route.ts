...
export const GET = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (error || !data) return Response.json({ error: 'Appariement de compétences introuvable' }, { status: 404 });
...
export const DELETE = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (error) return Response.json({ error: error.message }, { status: 500 });
  return Response.json({ success: true, message: 'Appariement de compétences supprimé' });
});