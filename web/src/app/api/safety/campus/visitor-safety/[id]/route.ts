...
export const GET = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (error || !record) return Response.json({ error: 'Sécurité visiteur introuvable' }, { status: 404 });
...
export const PUT = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (!['ADMIN', 'SUPER_ADMIN', 'DIRECTEUR', 'SURVEILLANT'].includes(profile?.role)) {
...
export const DELETE = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  const { error } = await supabase.from('visitor_safety').delete().eq('id', id).eq('school_id', ctx.schoolId);
