...
export const GET = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  const { data, error } = await supabase
    .from('workforce_ai_orchestrator')
    .select('*')
    .eq('id', id)
  ...
  if (error || !data) return Response.json({ error: 'Orchestrateur IA introuvable' }, { status: 404 });
...
export const DELETE = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (error) return Response.json({ error: error.message }, { status: 500 });
  return Response.json({ success: true, message: 'Orchestrateur IA supprimé' });
});