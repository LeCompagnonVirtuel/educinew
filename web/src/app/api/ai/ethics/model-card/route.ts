...
import { modelCardmodelCardSchema } from '@/features/ai/validators/modelCardSchemas';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await service.getModelCards(schoolId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = modelCardSchema.parse(body);
    const service = new AiEthicsService(supabase);
    const data = await service.POST_getModelCards(validated);
