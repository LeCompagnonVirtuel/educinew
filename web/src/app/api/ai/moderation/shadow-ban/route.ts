...
import { shadowBanshadowBanSchema } from '@/features/ai/validators/shadowBanSchemas';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = shadowBanSchema.parse(body);
    const service = new AiModerationService(supabase);
    const data = await service.applyShadowBan(validated);
