...
import { rbacrbacSchema } from '@/features/ai/validators/rbacSchemas';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await service.getRBAC(schoolId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = rbacSchema.parse(body);
    const service = new AiAuthorizationService(supabase);
    const data = await service.POST_getRBAC(validated);
