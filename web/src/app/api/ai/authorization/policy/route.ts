...
import { policypolicySchema } from '@/features/ai/validators/policySchemas';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await service.getPolicies(schoolId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const service = new AiAuthorizationService(supabase);
    const data = await service.POST_getPolicies(validated);
