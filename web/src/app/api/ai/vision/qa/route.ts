...
import { visualQAvisualQASchema } from '@/features/ai/validators/visualQASchemas';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = visualQASchema.parse(body);
    const service = new AiVisionProcessingService(supabase);
    const data = await service.visualQuestionAnswer(validated);
