...
import { textDetectiontextDetectionSchema } from '@/features/ai/validators/textDetectionSchemas';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = textDetectionSchema.parse(body);
    const service = new AiVisionProcessingService(supabase);
    const data = await service.detectText(validated);
