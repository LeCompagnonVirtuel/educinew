...
import { dashboardAlertdashboardAlertSchema } from '@/features/ai/validators/dashboardAlertSchemas';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await service.getAlerts(schoolId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = dashboardAlertSchema.parse(body);
    const service = new AiDashboardService(supabase);
    const data = await service.POST_getAlerts(validated);
