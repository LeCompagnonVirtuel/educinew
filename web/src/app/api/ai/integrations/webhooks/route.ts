...
import { integrationWebhookintegrationWebhookSchema } from '@/features/ai/validators/integrationWebhookSchemas';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await service.getWebhooks(schoolId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = integrationWebhookSchema.parse(body);
    const service = new AiIntegrationService(supabase);
    const data = await service.POST_getWebhooks(validated);
