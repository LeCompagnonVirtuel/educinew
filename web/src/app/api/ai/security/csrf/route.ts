...
import { csrfcsrfSchema } from '@/features/ai/validators/csrfSchemas';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await service.getCsrfConfig(schoolId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = csrfSchema.parse(body);
    const service = new AiSecurityService(supabase);
    const data = await service.POST_getCsrfConfig(validated);
