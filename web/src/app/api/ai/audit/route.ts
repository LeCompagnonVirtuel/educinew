...
import { auditEventauditEventSchema } from '@/features/ai/validators/auditEventSchemas';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const service = new AiAuditService(supabase);
    const data = await service.listAuditEvents(schoolId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = auditEventSchema.parse(body);
    const service = new AiAuditService(supabase);
    const data = await service.POST_listAuditEvents(validated);
