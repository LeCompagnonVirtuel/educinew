...
import { consentManagementconsentManagementSchema } from '@/features/ai/validators/consentManagementSchemas';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const service = new AiComplianceService(supabase);
    const data = await service.getConsents(schoolId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = consentManagementSchema.parse(body);
    const service = new AiComplianceService(supabase);
    const data = await service.POST_getConsents(validated);
