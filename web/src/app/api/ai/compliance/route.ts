...
import { compliancecomplianceSchema } from '@/features/ai/validators/complianceSchemas';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await service.listCompliance(schoolId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = complianceSchema.parse(body);
    const service = new AiComplianceService(supabase);
    const data = await service.POST_listCompliance(validated);
