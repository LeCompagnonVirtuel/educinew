...
import { piiDetectionpiiDetectionSchema } from '@/features/ai/validators/piiDetectionSchemas';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = piiDetectionSchema.parse(body);
    const service = new AiSafetyService(supabase);
    const data = await service.detectPII(validated);
