...
import { keyManagementkeyManagementSchema } from '@/features/ai/validators/keyManagementSchemas';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const service = new AiEncryptionService(supabase);
    const data = await service.getKeys(schoolId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = keyManagementSchema.parse(body);
    const service = new AiEncryptionService(supabase);
    const data = await service.POST_getKeys(validated);
