...
import { notificationDigestnotificationDigestSchema } from '@/features/ai/validators/notificationDigestSchemas';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await service.getDigests(schoolId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validated = notificationDigestSchema.parse(body);
    const service = new AiNotificationService(supabase);
    const data = await service.POST_getDigests(validated);
