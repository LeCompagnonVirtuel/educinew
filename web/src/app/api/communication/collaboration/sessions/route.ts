...
import { createCollaborationService } from '@/features/communication/services/collaboration.service';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await service.listSessions(params);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const repo = createCommunicationRepository(supabase);
    const service = createCollaborationService(repo);
    const data = await service.startSession(body);
