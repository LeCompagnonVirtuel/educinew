...
import { createPresenceService } from '@/features/communication/services/presence.service';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const service = createPresenceService(repo);
    const { searchParams } = new URL(request.url);
    const params = Object.fromEntries(searchParams);
    const data = await service.listPresence(params);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const repo = createCommunicationRepository(supabase);
    const service = createPresenceService(repo);
    const data = await service.updatePresence(body);
    return NextResponse.json({ data });
  } catch (error: unknown) {
    const err = error instanceof Error ? error : new Error(String(error));
    return NextResponse.json({ error: err.message }, { status: (error as { statusCode?: number }).statusCode || 500 });
