...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.exportDocuments(params.schoolId, params.userId, params.documentIds?.split(','), params.format);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const repo = createCommunicationRepository(supabase);
    const service = createExportService(repo);
    const data = await service.exportDocuments(body.schoolId, body.userId, body.documentIds, body.format);
