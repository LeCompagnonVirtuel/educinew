...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await service.getNotifications(params.schoolId, params.userId, params);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const repo = createCommunicationRepository(supabase);
    const service = createNotificationService(repo);
    const data = await service.sendNotificationBatch(body.schoolId, body.userId, body);
