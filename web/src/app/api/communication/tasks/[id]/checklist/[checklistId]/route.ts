...
export async function POST(request: NextRequest, { params }: { params: { id: string; checklistId: string } }) {
  try {
    const body = await request.json();
    const supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!);
    const repo = createCommunicationRepository(supabase);
    const service = createTaskService(repo);
    const data = await service.toggleChecklistItem(params.id, params.checklistId, body);
