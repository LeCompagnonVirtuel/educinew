...
import { createPollService } from '@/features/communication/services/poll.service';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const service = createPollService(repo);
    const { searchParams } = new URL(request.url);
    const params = Object.fromEntries(searchParams);
    const data = await service.listPolls(params);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const repo = createCommunicationRepository(supabase);
    const service = createPollService(repo);
    const data = await service.createPoll(body);
