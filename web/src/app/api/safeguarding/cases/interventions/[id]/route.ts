...
export const GET = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (error || !record) return Response.json({ error: 'Intervention introuvable' }, { status: 404 });
...
export const PUT = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (!['SURVEILLANT', 'ADMIN', 'SUPER_ADMIN', 'DIRECTEUR', 'SECRETAIRE'].includes(profile?.role)) {
...
export const DELETE = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  const { error } = await supabase.from('interventions').delete().eq('id', id).eq('school_id', ctx.schoolId);
