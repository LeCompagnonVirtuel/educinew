...
import { createClient } from '@/utils/supabase/server';
  ...
  try {
    ...
    const service = new InteroperabilityEventService(supabase);
    const data = await service.getEvent(profile.school_id, id);
    if (!data) return NextResponse.json({ error: 'Ã‰vÃ©nement introuvable' }, { status: 404 });

    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET interoperability/events/[id]', error);
...
export async function PUT(
  ...
  try {
    ...
    const service = new InteroperabilityEventService(supabase);
    const data = await service.updateEvent(profile.school_id, id, validation.data);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur PUT interoperability/events/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const service = new InteroperabilityEventService(supabase);
    await service.deleteEvent(profile.school_id, id);
    return NextResponse.json({ success: true });
  } catch (error) {
    logger.error('Erreur DELETE interoperability/events/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}