...
import { createCredentialSchema } from '@/features/gei2p/validators';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listCredentials(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET interoperability/credentials', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const service = new InteroperabilityCredentialService(supabase);
    const data = await service.createCredential(profile.school_id, user.id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST interoperability/credentials', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
