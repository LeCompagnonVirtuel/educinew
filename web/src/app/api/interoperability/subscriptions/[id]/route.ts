...
import { updateSubscriptionSchema } from '@/features/gei2p/validators';
...
export async function GET(
  ...
  try {
    ...
    const data = await service.getSubscription(profile.school_id, id);
    if (!data) return NextResponse.json({ error: 'Abonnement introuvable' }, { status: 404 });

    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET interoperability/subscriptions/[id]', error);
...
export async function PUT(
  ...
  try {
    ...
    const service = new InteroperabilitySubscriptionService(supabase);
    const data = await service.updateSubscription(profile.school_id, id, validation.data);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur PUT interoperability/subscriptions/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const service = new InteroperabilitySubscriptionService(supabase);
    await service.deleteSubscription(profile.school_id, id);
    return NextResponse.json({ success: true });
  } catch (error) {
    logger.error('Erreur DELETE interoperability/subscriptions/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}