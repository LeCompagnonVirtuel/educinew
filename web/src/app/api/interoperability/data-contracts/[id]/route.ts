...
export async function GET(
  ...
  try {
    ...
    const service = new InteroperabilityDataContractService(supabase);
    const data = await service.getDataContract(profile.school_id, id);
    if (!data) return NextResponse.json({ error: 'Contrat de donn̩es introuvable' }, { status: 404 });

    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET interoperability/data-contracts/[id]', error);
...
export async function PUT(
  ...
  try {
    ...
    const validation = updateDataContractSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: 'Donn̩es invalides', details: validation.error.flatten().fieldErrors }, { status: 400 });
    }

    const service = new InteroperabilityDataContractService(supabase);
    const data = await service.updateDataContract(profile.school_id, id, validation.data);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur PUT interoperability/data-contracts/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const service = new InteroperabilityDataContractService(supabase);
    await service.deleteDataContract(profile.school_id, id);
    return NextResponse.json({ success: true });
  } catch (error) {
    logger.error('Erreur DELETE interoperability/data-contracts/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}