...
import { createDataContractSchema } from '@/features/gei2p/validators';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listDataContracts(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET interoperability/data-contracts', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = createDataContractSchema.safeParse(body);
    if (!validation.success) {
      return NextResponse.json({ error: 'Données invalides', details: validation.error.flatten().fieldErrors }, { status: 400 });
    }

    const service = new InteroperabilityDataContractService(supabase);
    const data = await service.createDataContract(profile.school_id, user.id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST interoperability/data-contracts', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
