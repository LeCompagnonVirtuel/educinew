...
import { createIdentitySchema } from '@/features/gei2p/validators';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await service.listIdentities(profile.school_id, filters);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET interoperability/identities', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const service = new InteroperabilityIdentityService(supabase);
    const data = await service.createIdentity(profile.school_id, user.id, validation.data);
    return NextResponse.json({ data }, { status: 201 });
  } catch (error) {
    logger.error('Erreur POST interoperability/identities', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
