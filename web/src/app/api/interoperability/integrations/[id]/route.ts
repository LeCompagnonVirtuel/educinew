...
import { updateIntegrationSchema } from '@/features/gei2p/validators';
...
export async function GET(
  ...
  try {
    ...
    const data = await service.getIntegration(profile.school_id, id);
    if (!data) return NextResponse.json({ error: 'Int̩gration introuvable' }, { status: 404 });

    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET interoperability/integrations/[id]', error);
...
export async function PUT(
  ...
  try {
    ...
    const service = new InteroperabilityIntegrationService(supabase);
    const data = await service.updateIntegration(profile.school_id, id, validation.data);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur PUT interoperability/integrations/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const service = new InteroperabilityIntegrationService(supabase);
    await service.deleteIntegration(profile.school_id, id);
    return NextResponse.json({ success: true });
  } catch (error) {
    logger.error('Erreur DELETE interoperability/integrations/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}