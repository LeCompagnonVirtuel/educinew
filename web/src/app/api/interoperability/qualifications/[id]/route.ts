...
import { updateQualificationSchema } from '@/features/gei2p/validators';
...
export async function GET(
  ...
  try {
    ...
    const data = await service.getQualification(profile.school_id, id);
    if (!data) return NextResponse.json({ error: 'Qualification introuvable' }, { status: 404 });

    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur GET interoperability/qualifications/[id]', error);
...
export async function PUT(
  ...
  try {
    ...
    const service = new InteroperabilityQualificationService(supabase);
    const data = await service.updateQualification(profile.school_id, id, validation.data);
    return NextResponse.json({ data });
  } catch (error) {
    logger.error('Erreur PUT interoperability/qualifications/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const service = new InteroperabilityQualificationService(supabase);
    await service.deleteQualification(profile.school_id, id);
    return NextResponse.json({ success: true });
  } catch (error) {
    logger.error('Erreur DELETE interoperability/qualifications/[id]', error);
    const message = error instanceof Error ? error.message : 'Erreur interne du serveur';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
