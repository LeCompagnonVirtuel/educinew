...
import { createPermissionsService } from '@/features/analytics/services/permissions.service';
...
export async function GET(request: NextRequest, { params }: { params: { resource: string } }) {
  try {
    ...
    const service = createPermissionsService(repo);
    const data = await service.getPermissionsByResource(params.resource);
...
export async function PUT(request: NextRequest, { params }: { params: { resource: string } }) {
  try {
    ...
    const data = await service.updatePermissionsByResource(params.resource, body);
    return NextResponse.json({ data });
  } catch (error: any) {
    return NextResponse.json({ error: error.message }, { status: error.statusCode || 500 });
  }
...
export async function DELETE(request: NextRequest, { params }: { params: { resource: string } }) {
  try {
    ...
    const repo = createAnalyticsRepository(supabase);
    const service = createPermissionsService(repo);
    await service.deletePermissionsByResource(params.resource);
