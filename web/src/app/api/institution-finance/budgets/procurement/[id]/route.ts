...
export async function GET(
  ...
  try {
    ...
    if (error || !data) {
      return NextResponse.json({ error: 'Approvisionnement non trouvÃƒÂ©' }, { status: 404 });
    }
...
export async function PUT(
  ...
  try {
    ...
    const validation = procurementUpdateSchema.safeParse(body);
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur mise ÃƒÂ  jour Approvisionnement: ${error.message}` }, { status: 500 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur suppression Approvisionnement: ${error.message}` }, { status: 500 });
    }

    return NextResponse.json({ message: 'Approvisionnement supprimÃƒÂ© avec succÃƒÂ¨s' });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Erreur serveur' },
      { status: 500 }
