...
import { UpdateEmploymentSchema } from '@/features/gegin/validators/employment-validators';
...
import { createClient } from '@/utils/supabase/server';
  try {
    ...
    if (!result) return NextResponse.json({ error: 'Employment introuvable' }, { status: 404 });

    return NextResponse.json(result);
  } catch (error) {
    logger.error('Erreur lors de la rÃ©cupÃ©ration de l\'employment:', error);
...
export async function PUT(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validation = UpdateEmploymentSchema.safeParse(body);
    ...
    logger.error('Erreur lors de la mise Ã  jour de l\'employment:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    logger.error('Erreur lors de la suppression de l\'employment:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}