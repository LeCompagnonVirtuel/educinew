...
import { CreateAnalyticsSchema } from '@/features/gegin/validators/analytics-validators';
...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error('Erreur lors de la liste des configs analytics:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = CreateAnalyticsSchema.safeParse({ ...body, school_id: schoolId });
    ...
    logger.error('Erreur lors de la cr̩ation de la config analytics:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
