...
import { CreateOrganizationSchema } from '@/features/gegin/validators/organizations-validators';
import { createCrudRepository } from '@/features/gegin/repositories/gegin-base.repository';

const TABLE = 'gegin_organizations';
...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error('Erreur lors de la liste des organisations:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = CreateOrganizationSchema.safeParse({ ...body, school_id: schoolId });
    ...
    logger.error('Erreur lors de la cr̩ation de l\'organisation:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
