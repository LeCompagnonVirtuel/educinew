...
import { CreateComplianceSchema } from '@/features/gegin/validators/compliance-validators';
...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error('Erreur lors de la liste des conformit̩s:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = CreateComplianceSchema.safeParse({ ...body, school_id: schoolId });
    ...
    logger.error('Erreur lors de la cr̩ation de la conformit̩:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
