...
import { CreateMarketplaceSchema } from '@/features/gegin/validators/marketplace-validators';
...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error('Erreur lors de la liste du marketplace:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = CreateMarketplaceSchema.safeParse({ ...body, school_id: schoolId });
    ...
    logger.error('Erreur lors de la cr̩ation de l\'article marketplace:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
