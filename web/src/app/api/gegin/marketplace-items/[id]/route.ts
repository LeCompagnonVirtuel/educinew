...
import { UpdateMarketplaceSchema } from '@/features/gegin/validators/marketplace-validators';
import { createCrudRepository } from '@/features/gegin/repositories/gegin-base.repository';

const TABLE = 'gegin_marketplace_items';
...
import { createClient } from '@/utils/supabase/server';
  try {
    ...
    if (!result) return NextResponse.json({ error: 'Article marketplace introuvable' }, { status: 404 });

    return NextResponse.json(result);
  } catch (error) {
    logger.error('Erreur lors de la rÃ©cupÃ©ration de l\'article marketplace:', error);
...
export async function PUT(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    logger.error('Erreur lors de la mise Ã  jour de l\'article marketplace:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    logger.error('Erreur lors de la suppression de l\'article marketplace:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}