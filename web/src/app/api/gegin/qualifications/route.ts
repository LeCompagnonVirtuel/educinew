...
import { CreateQualificationSchema } from '@/features/gegin/validators/qualifications-validators';
...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error('Erreur lors de la liste des qualifications:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = CreateQualificationSchema.safeParse({ ...body, school_id: schoolId });
    ...
    logger.error('Erreur lors de la cr̩ation de la qualification:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
