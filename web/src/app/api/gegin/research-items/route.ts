...
import { CreateResearchSchema } from '@/features/gegin/validators/research-validators';
import { createCrudRepository } from '@/features/gegin/repositories/gegin-base.repository';

const TABLE = 'gegin_research_items';
  ...
  try {
    ...
    const filtered = search
      ? data.filter((item: Record<string, unknown>) =>
          (item.title as string)?.toLowerCase().includes(search.toLowerCase()))
      : data;
    ...
    logger.error('Erreur lors de la liste des recherches:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!['ADMIN', 'SUPER_ADMIN', 'ENSEIGNANT'].includes(role)) {
      return NextResponse.json({ error: 'Non autorisÃ©' }, { status: 403 });
    }
    if (!schoolId) return NextResponse.json({ error: 'Ã‰tablissement requis' }, { status: 403 });

    const body = await req.json();
    const validation = CreateResearchSchema.safeParse({ ...body, school_id: schoolId });
    ...
    logger.error('Erreur lors de la crÃ©ation de la recherche:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
