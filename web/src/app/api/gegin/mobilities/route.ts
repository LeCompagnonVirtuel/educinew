...
import { CreateMobilitySchema } from '@/features/gegin/validators/mobility-validators';
...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error('Erreur lors de la liste des mobilit̩s:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = CreateMobilitySchema.safeParse({ ...body, school_id: schoolId });
    ...
    logger.error('Erreur lors de la cr̩ation de la mobilit̩:', error);
    return NextResponse.json({ error: 'Erreur interne du serveur' }, { status: 500 });
  }
}
