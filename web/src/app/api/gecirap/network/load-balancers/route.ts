...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('load_balancers').select('*', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('load_balancers').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      type: data.type, algorithm: data.algorithm, targets: data.targets || [],
      health_check: data.healthCheck || {}, status: data.status || 'PENDING', created_by: user.id,
    }).select().single();
