...
const CreateSchema = z.object({
  ...
  type: z.enum(['PUBLIC', 'PRIVATE', 'VPC', 'SUBNET']),
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('networks').select('*', { count: 'exact' }).eq('school_id', schoolId).is('deleted_at', null);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('networks').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      cidr: data.cidr || null, type: data.type, config: data.config || {},
