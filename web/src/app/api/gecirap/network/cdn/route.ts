...
const CreateSchema = z.object({
  ...
  origin: z.string().url('URL origine invalide'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('cdn_distributions').select('*', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('cdn_distributions').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      domain: data.domain, origin: data.origin, config: data.config || {},
