...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('edge_deployments').select('*, cluster:edge_clusters(id, name), node:edge_nodes(id, name)', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('edge_deployments').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      cluster_id: data.clusterId || null, node_id: data.nodeId || null,
      package_version: data.packageVersion || null, config: data.config || {},
      status: data.status || 'PENDING', created_by: user.id,
    }).select().single();
