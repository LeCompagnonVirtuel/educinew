...
const CreateSchema = z.object({
  ...
  targetNodes: z.array(z.string().uuid()).optional(),
  config: z.record(z.unknown()).optional(),
  status: z.enum(['DRAFT', 'PENDING', 'DEPLOYED', 'FAILED']).optional(),
});
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('edge_offline_packages').select('*', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('edge_offline_packages').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      version: data.version || '1.0.0', content: data.content || null,
      target_nodes: data.targetNodes || [], config: data.config || {},
