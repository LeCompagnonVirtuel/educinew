...
const CreateSchema = z.object({
  environmentId: z.string().uuid('ID environnement invalide'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      ...
      .select('*, environment:cloud_environments(id, name, type)', { count: 'exact' })
      .eq('school_id', schoolId)
      .is('deleted_at', null);

    if (filters.environment_id) query = query.eq('environment_id', filters.environment_id);
