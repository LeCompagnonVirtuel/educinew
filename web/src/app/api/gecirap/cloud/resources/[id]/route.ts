...
const UpdateSchema = z.object({
  ...
  type: z.enum(['VM', 'CONTAINER', 'DATABASE', 'STORAGE', 'NETWORK', 'FUNCTION', 'OTHER']).optional(),
...
export async function GET(
  ...
  try {
    ...
    const { data: record, error } = await supabase
      ...
      .select('*, provider:cloud_providers(id, name, type), account:cloud_accounts(id, name)')
      .eq('id', id)
      .is('deleted_at', null)
      .single();

    if (error || !record) return NextResponse.json({ error: 'Ressource introuvable' }, { status: 404 });
