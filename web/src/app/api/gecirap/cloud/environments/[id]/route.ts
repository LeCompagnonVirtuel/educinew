...
import { createClient } from '@/utils/supabase/server';
  ...
  type: z.enum(['PRODUCTION', 'STAGING', 'DEVELOPMENT', 'TESTING', 'DR']).optional(),
...
export async function GET(
  ...
  try {
    ...
    if (error || !record) return NextResponse.json({ error: 'Environnement introuvable' }, { status: 404 });
...
export async function DELETE(
  ...
  try {
    ...
    if (!existing) return NextResponse.json({ error: 'Environnement introuvable' }, { status: 404 });
