...
const UpdateSchema = z.object({
  ...
  type: z.enum(['DEPLOYMENT', 'STATEFULSET', 'DAEMONSET', 'CRONJOB', 'JOB']).optional(),
...
export async function GET(
  ...
  try {
    ...
    const { data: record, error } = await supabase
      ...
      .select('*, cluster:container_clusters(id, name, engine)')
      .eq('id', id)
      .is('deleted_at', null)
      .single();

    if (error || !record) return NextResponse.json({ error: 'Workload introuvable' }, { status: 404 });
...
export async function DELETE(
  ...
  try {
    ...
    if (!existing) return NextResponse.json({ error: 'Workload introuvable' }, { status: 404 });
