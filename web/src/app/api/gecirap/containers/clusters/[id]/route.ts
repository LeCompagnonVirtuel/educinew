...
const UpdateSchema = z.object({
  ...
  engine: z.enum(['KUBERNETES', 'DOCKER_SWARM', 'ECS', 'AKS', 'GKE', 'OPENSHIFT']).optional(),
  endpoint: z.string().url().optional(),
  config: z.record(z.unknown()).optional(),
  status: z.enum(['PROVISIONING', 'ACTIVE', 'INACTIVE', 'ERROR']).optional(),
});
...
export async function GET(
  ...
  try {
    ...
    const { data: record, error } = await supabase
      ...
      .select('*, provider:cloud_providers(id, name, type)')
      .eq('id', id)
      .is('deleted_at', null)
      .single();

    if (error || !record) return NextResponse.json({ error: 'Cluster introuvable' }, { status: 404 });
...
export async function DELETE(
  ...
  try {
    ...
    if (!existing) return NextResponse.json({ error: 'Cluster introuvable' }, { status: 404 });
