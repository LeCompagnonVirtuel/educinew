...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('iac_stacks').select('*', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('iac_stacks').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      template_id: data.templateId || null, environment_id: data.environmentId || null,
      variables: data.variables || {}, status: data.status || 'PENDING', created_by: user.id,
    }).select().single();
