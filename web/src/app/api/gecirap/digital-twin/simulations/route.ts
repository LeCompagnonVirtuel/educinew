...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('twin_simulations').select('*, twin:digital_twins(id, name, type)', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('twin_simulations').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      twin_id: data.twinId, scenario_id: data.scenarioId || null,
      config: data.config || {}, status: data.status || 'PENDING', created_by: user.id,
    }).select().single();
