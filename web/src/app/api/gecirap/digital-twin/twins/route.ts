...
const CreateSchema = z.object({
  ...
  type: z.enum(['INFRASTRUCTURE', 'NETWORK', 'APPLICATION', 'STUDENT', 'SCHOOL']),
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('digital_twins').select('*', { count: 'exact' }).eq('school_id', schoolId).is('deleted_at', null);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('digital_twins').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      type: data.type, source_id: data.sourceId || null, config: data.config || {},
