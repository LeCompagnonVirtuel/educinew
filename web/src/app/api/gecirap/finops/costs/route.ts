...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!['SUPER_ADMIN', 'ADMIN', 'COMPTABLE'].includes(profile?.role)) return NextResponse.json({ error: 'Non autorise' }, { status: 403 });
    const url = new URL(request.url);
    const page = parseInt(url.searchParams.get('page') || '1');
    const limit = parseInt(url.searchParams.get('limit') || '20');
    const offset = (page - 1) * limit;
    const { data, error, count } = await supabase
      .from('finops_costs').select('*, provider:cloud_providers(id, name, type)', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('finops_costs').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      provider_id: data.providerId || null, amount: data.amount,
      currency: data.currency || 'XOF', period: data.period,
      category: data.category || null, metadata: data.metadata || {},
      status: data.status || 'ACTIVE', created_by: user.id,
    }).select().single();
