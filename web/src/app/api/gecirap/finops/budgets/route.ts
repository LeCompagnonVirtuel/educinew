...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('finops_budgets').select('*', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('finops_budgets').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      amount: data.amount, period: data.period,
      alert_threshold: data.alertThreshold || 80, status: data.status || 'ACTIVE', created_by: user.id,
    }).select().single();
