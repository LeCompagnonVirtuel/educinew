...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('autoscaling_policies').select('*', { count: 'exact' }).eq('school_id', schoolId).is('deleted_at', null);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('autoscaling_policies').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      type: data.type, min_replicas: data.minReplicas, max_replicas: data.maxReplicas,
      target_value: data.targetValue, cooldown_seconds: data.cooldownSeconds || 300,
