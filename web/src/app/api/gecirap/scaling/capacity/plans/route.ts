...
const CreateSchema = z.object({
  ...
  type: z.enum(['SCALE_UP', 'SCALE_DOWN', 'RESIZE']),
  targetResource: z.string().optional(),
  config: z.record(z.unknown()).optional(),
  scheduledAt: z.string().optional(),
  status: z.enum(['DRAFT', 'SCHEDULED', 'IN_PROGRESS', 'COMPLETED', 'FAILED']).optional(),
});
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('capacity_plans').select('*', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('capacity_plans').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      type: data.type, target_resource: data.targetResource || null,
      config: data.config || {}, scheduled_at: data.scheduledAt || null,
