...
const CreateSchema = z.object({
  ...
  type: z.enum(['AUTOMATIC', 'SEMI_AUTOMATIC', 'MANUAL']),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('aiops_remediation').select('*', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('aiops_remediation').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      type: data.type, trigger_condition: data.triggerCondition || {},
      actions: data.actions || [], config: data.config || {},
