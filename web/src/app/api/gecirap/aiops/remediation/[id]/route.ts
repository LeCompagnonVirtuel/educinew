...
const UpdateSchema = z.object({
  ...
  type: z.enum(['AUTOMATIC', 'SEMI_AUTOMATIC', 'MANUAL']).optional(),
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: record, error } = await supabase.from('aiops_remediation').select('*').eq('id', id).is('deleted_at', null).single();
    if (error || !record) return NextResponse.json({ error: 'Remediation introuvable' }, { status: 404 });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: record, error } = await supabase.from('aiops_remediation').update(updateData).eq('id', id).eq('school_id', profile?.school_id).select().single();
