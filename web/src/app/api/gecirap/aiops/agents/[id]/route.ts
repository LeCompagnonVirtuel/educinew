...
const UpdateSchema = z.object({
  ...
  type: z.enum(['ANOMALY_DETECTION', 'PREDICTIVE', 'RECOMMENDATION', 'REMEDIATION']).optional(),
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: record, error } = await supabase.from('aiops_agents').select('*').eq('id', id).is('deleted_at', null).single();
    if (error || !record) return NextResponse.json({ error: 'Agent AIOps introuvable' }, { status: 404 });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: record, error } = await supabase.from('aiops_agents').update(updateData).eq('id', id).eq('school_id', profile?.school_id).select().single();
