...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('aiops_events').select('*', { count: 'exact' }).eq('school_id', schoolId);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('aiops_events').insert({
      school_id: schoolId, title: data.title, description: data.description || null,
      severity: data.severity, source: data.source || null, source_id: data.sourceId || null,
      metadata: data.metadata || {}, status: data.status || 'OPEN', created_by: user.id,
    }).select().single();
