...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('multicloud_balances').select('*, provider:cloud_providers(id, name, type)', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('multicloud_balances').insert({
      school_id: schoolId, name: data.name, provider_id: data.providerId || null,
      target_spend: data.targetSpend, period: data.period,
