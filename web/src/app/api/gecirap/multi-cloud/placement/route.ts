...
const CreateSchema = z.object({
  ...
  strategy: z.enum(['COST_OPTIMIZED', 'PERFORMANCE', 'COMPLIANCE', 'AVAILABILITY']),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('multicloud_placements').select('*, provider:cloud_providers(id, name, type)', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('multicloud_placements').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      provider_id: data.providerId || null, resource_type: data.resourceType || null,
      strategy: data.strategy, config: data.config || {},
