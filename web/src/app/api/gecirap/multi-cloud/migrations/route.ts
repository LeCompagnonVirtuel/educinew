...
const CreateSchema = z.object({
  ...
  strategy: z.enum(['LIVE', 'SCHEDULED', 'MANUAL']),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('multicloud_migrations').select('*, source_provider:cloud_providers!multicloud_migrations_source_provider_id_fkey(id, name), target_provider:cloud_providers!multicloud_migrations_target_provider_id_fkey(id, name)', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('multicloud_migrations').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      source_provider_id: data.sourceProviderId, target_provider_id: data.targetProviderId,
      resource_ids: data.resourceIds, strategy: data.strategy,
      config: data.config || {}, status: data.status || 'DRAFT', created_by: user.id,
    }).select().single();
