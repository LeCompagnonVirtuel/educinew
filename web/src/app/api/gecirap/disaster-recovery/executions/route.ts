...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('dr_executions').select('*, plan:dr_plans(id, name, type)', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('dr_executions').insert({
      school_id: schoolId, plan_id: data.planId, name: data.name,
      triggered_by: data.triggeredBy || user.id, notes: data.notes || null,
      status: data.status || 'IN_PROGRESS', created_by: user.id,
    }).select().single();
