...
const CreateSchema = z.object({
  ...
  type: z.enum(['FULL', 'PARTIAL', 'TABLETOP']),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error, count } = await supabase
      .from('dr_tests').select('*, plan:dr_plans(id, name, type)', { count: 'exact' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('dr_tests').insert({
      school_id: schoolId, plan_id: data.planId, name: data.name,
      type: data.type, config: data.config || {},
      status: data.status || 'SCHEDULED', created_by: user.id,
    }).select().single();
