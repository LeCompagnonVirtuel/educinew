...
const CreateSchema = z.object({
  ...
  type: z.enum(['FULL', 'PARTIAL', 'DATABASE', 'FILESYSTEM']),
  rtoMinutes: z.number().int().min(1),
  rpoMinutes: z.number().int().min(0),
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('dr_plans').select('*', { count: 'exact' }).eq('school_id', schoolId).is('deleted_at', null);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: record, error } = await supabase.from('dr_plans').insert({
      school_id: schoolId, name: data.name, description: data.description || null,
      type: data.type, rto_minutes: data.rtoMinutes, rpo_minutes: data.rpoMinutes,
      resources: data.resources || [], schedule: data.schedule || null,
      status: data.status || 'DRAFT', created_by: user.id,
    }).select().single();
