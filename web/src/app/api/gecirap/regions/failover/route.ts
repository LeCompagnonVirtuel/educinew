...
const CreateSchema = z.object({
  ...
  sourceRegionId: z.string().uuid('ID source invalide'),
  targetRegionId: z.string().uuid('ID cible invalide'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      .from('failover_rules')
      .select('*, source_region:geo_regions!failover_rules_source_region_id_fkey(id, name, code), target_region:geo_regions!failover_rules_target_region_id_fkey(id, name, code)', { count: 'exact' })
  ...
  try {
    ...
    const { data: record, error } = await supabase
      ...
      .insert({
        ...
        target_region_id: data.targetRegionId,
        type: data.type,
