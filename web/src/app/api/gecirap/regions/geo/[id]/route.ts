...
export async function GET(
  ...
  try {
    ...
    if (error || !record) return NextResponse.json({ error: 'Region geographique introuvable' }, { status: 404 });
...
export async function DELETE(
  ...
  try {
    ...
    if (!existing) return NextResponse.json({ error: 'Region geographique introuvable' }, { status: 404 });
