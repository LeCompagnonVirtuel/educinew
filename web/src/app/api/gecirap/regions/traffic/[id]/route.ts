...
export async function GET(
  ...
  try {
    ...
    const { data: record, error } = await supabase
      ...
      .select('*, region:geo_regions(id, name, code)')
      .eq('id', id)
      .is('deleted_at', null)
      .single();

    if (error || !record) return NextResponse.json({ error: 'Politique de trafic introuvable' }, { status: 404 });
...
export async function PUT(
  ...
  try {
    ...
    if (data.regionId !== undefined) updateData.region_id = data.regionId;
...
export async function DELETE(
  ...
  try {
    ...
    if (!existing) return NextResponse.json({ error: 'Politique de trafic introuvable' }, { status: 404 });
