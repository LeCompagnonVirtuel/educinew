...
export const GET = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (error || !record) return Response.json({ error: 'Orientation conseiller introuvable' }, { status: 404 });
...
export const PUT = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  if (!['INFIRMIER', 'ADMIN', 'SUPER_ADMIN', 'DIRECTEUR', 'ENSEIGNANT'].includes(profile?.role)) {
...
export const DELETE = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  const { error } = await supabase.from('counselor_referrals').delete().eq('id', id).eq('school_id', ctx.schoolId);
