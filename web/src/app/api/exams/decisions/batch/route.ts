...
import { decisionSchema } from '@/features/exams/validators/schemas';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!Array.isArray(body.decisions)) {
      return NextResponse.json({ error: 'Le champ decisions doit être un tableau' }, { status: 400 });
    }

    const results = [];
    for (const item of body.decisions) {
      const validation = decisionSchema.safeParse({ ...item, schoolId });
      ...
      if (error) {
        ...
        results.push({ studentId: item.studentId, success: true, decision });
      }
    }
    ...
    logger.error('Error batch creating decisions', error);
