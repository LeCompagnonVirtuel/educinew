...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = classRankingSchema.safeParse({ ...body, schoolId });
    ...
    return NextResponse.json({ success: true, message: 'Classement calculé' }, { status: 201 });
  } catch (error) {
    logger.error('Error creating ranking', error);
    return NextResponse.json({ error: 'Erreur interne' }, { status: 500 });
  }
}
