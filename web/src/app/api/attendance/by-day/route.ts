...
export const GET = withSupabase({ auth: 'user' }, async (req, ctx) => {
  ...
  const validation = ByDaySchema.safeParse({
  ...
  for (const record of records) {
    if (!byDay[record.date]) {
      byDay[record.date] = { present: 0, absent: 0, late: 0, total: 0 };
    }
    byDay[record.date].total++;
    byDay[record.date][record.status.toLowerCase() as keyof typeof byDay[string]]++;
  }

  const result = Object.entries(byDay).map(([date, stats]) => ({
    date,
    ...stats,
