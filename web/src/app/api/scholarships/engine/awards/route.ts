...
const awardsSchema = z.object({
  application_id: z.string().optional().optional(),
...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur r̩cup̩ration Attribution de bourse: ${error.message}` }, { status: 500 });
    }
...
export async function POST(request: Request) {
  try {
    ...
    const validation = awardsSchema.safeParse(body);
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur cr̩ation Attribution de bourse: ${error.message}` }, { status: 500 });
    }
