...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur r̩cup̩ration Simulation de jumeau: ${error.message}` }, { status: 500 });
    }
...
export async function POST(request: Request) {
  try {
    ...
    const validation = simulationsSchema.safeParse(body);
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur cr̩ation Simulation de jumeau: ${error.message}` }, { status: 500 });
    }
