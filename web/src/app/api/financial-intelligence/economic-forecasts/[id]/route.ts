...
export async function GET(
  ...
  try {
    ...
    if (error || !data) {
      return NextResponse.json({ error: 'PrÃ©vision Ã©conomique non trouvÃ©' }, { status: 404 });
    }
...
export async function PUT(
  ...
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur mise Ã  jour PrÃ©vision Ã©conomique: ${error.message}` }, { status: 500 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur suppression PrÃ©vision Ã©conomique: ${error.message}` }, { status: 500 });
    }

    return NextResponse.json({ message: 'PrÃ©vision Ã©conomique supprimÃ© avec succÃ¨s' });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Erreur serveur' },
      { status: 500 }
