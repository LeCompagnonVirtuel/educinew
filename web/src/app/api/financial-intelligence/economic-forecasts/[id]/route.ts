...
export async function GET(
  ...
  try {
    ...
    if (error || !data) {
      return NextResponse.json({ error: 'PrÃƒÂ©vision ÃƒÂ©conomique non trouvÃƒÂ©' }, { status: 404 });
    }
...
export async function PUT(
  ...
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur mise ÃƒÂ  jour PrÃƒÂ©vision ÃƒÂ©conomique: ${error.message}` }, { status: 500 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      return NextResponse.json({ error: `Erreur suppression PrÃƒÂ©vision ÃƒÂ©conomique: ${error.message}` }, { status: 500 });
    }

    return NextResponse.json({ message: 'PrÃƒÂ©vision ÃƒÂ©conomique supprimÃƒÂ© avec succÃƒÂ¨s' });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Erreur serveur' },
      { status: 500 }
