...
import { etlPipelineCreateSchema } from '@/features/eduos/validators/eduos-data-validators';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await service.listETLPipelines(schoolId, filters);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation = etlPipelineCreateSchema.safeParse({ ...body, school_id: schoolId });
    ...
    const service = new EduOSETLPipelineService(supabase);
    const data = await service.createETLPipeline(schoolId, validation.data);
