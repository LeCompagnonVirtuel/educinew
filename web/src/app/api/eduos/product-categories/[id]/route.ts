...
import { productCategoryEntityUpdateSchema } from '@/features/eduos/validators/eduos-marketplace-validators';
...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const data = await service.getProductCategoryEntity(schoolId, id);
...
export async function PUT(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validation = productCategoryEntityUpdateSchema.safeParse(body);
    ...
    const service = new EduOSProductCategoryEntityService(supabase);
    const data = await service.updateProductCategoryEntity(schoolId, id, validation.data);
    return NextResponse.json(data);
  } catch (error) {
