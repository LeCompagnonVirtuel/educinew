...
export default function BulkImportTeachersPage() {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const res = await sbImport.downloadTeacherTemplate();
      const blob = await res.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'modele_import_professeurs_educi.csv';
      a.click();
  ...
  const handleValidate = async () => {
    ...
    setError('');
    try {
      const result = await sbImport.validateTeacherFile(file);
  ...
  const handleConfirm = async () => {
    ...
    try {
      const validRows = validationData.rows.filter((r) => r.valid).map((r) => ({
        ...
        email: r.email,
        matierePrincipale: r.matierePrincipale,
        classesEnseignees: r.classesEnseignees,
        statut: r.statut,
        adresse: r.adresse,
      }));
      const result = await sbImport.confirmTeacherImport(validRows);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const blob = await sbImport.exportTeacherAccessCardsPDF(importResult.credentials, exportBranding);
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'cartes_acces_professeurs_educi.pdf';
      a.click();
      window.URL.revokeObjectURL(url);
    } catch (err: any) {
      setError(err.message);
  ...
  return (
    <RoleLayout role="admin" breadcrumbs={[{ label: 'Personnel', href: '/teachers' }, { label: 'Import professeurs' }]}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-[#191c1d]">Import en masse de professeurs</h1>
          </div>
          <p className="text-[#464555]">Importez des dizaines ou centaines de professeurs en une seule opération.</p>
        </div>
        ...
        <div className="flex items-center justify-between max-w-2xl mb-10">
          {steps.map((s, i) => (
            <div key={s.id} className="flex items-center gap-2">
              ...
              <span className={`text-xs font-bold uppercase hidden sm:block ${i <= stepIndex ? 'text-[#3525cd]' : 'text-[#464555]'}`}>{s.label}</span>
            ...
            >
              ...
              {file ? (
                <div className="flex items-center justify-center gap-3 mb-4">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); setFile(null); }} className="ml-2 p-1 text-slate-400 hover:text-red-500"><X size={18} /></button>
                </div>
              ) : (
                <>
              ...
              <div className="flex gap-3 justify-center flex-wrap">
                <span className="px-4 py-2 bg-[#f3f4f5] rounded-full text-xs font-medium text-[#464555] flex items-center gap-2"><FileSpreadsheet size={14} /> CSV</span>
                <span className="px-4 py-2 bg-[#f3f4f5] rounded-full text-xs font-medium text-[#464555] flex items-center gap-2"><FileSpreadsheet size={14} /> XLSX</span>
                <span className="px-4 py-2 bg-[#f3f4f5] rounded-full text-xs font-medium text-[#464555]">Max 10MB</span>
              </div>
            </div>

            <div className="flex items-center justify-between">
              <button onClick={handleDownloadTemplate} className="flex items-center gap-2 px-5 py-2.5 text-[#3525cd] font-semibold text-sm hover:bg-indigo-50 rounded-xl transition-colors">
              ...
              <button onClick={handleValidate} disabled={!file || loading}
                ...
                {loading ? <Loader2 size={18} className="animate-spin" /> : <><span>Continuer</span> <ChevronRight size={18} /></>}
              </button>
            </div>
          </div>
        )}
          ...
          <div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
              <div className="bg-white rounded-xl p-5 border border-slate-100">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-2xl font-bold text-[#191c1d]">{validationData.totalRows}</p><p className="text-xs text-[#464555]">Total lignes</p></div>
                </div>
              </div>
              <div className="bg-white rounded-xl p-5 border border-slate-100">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-2xl font-bold text-emerald-600">{validationData.validRows}</p><p className="text-xs text-[#464555]">Valides</p></div>
                </div>
              </div>
              <div className="bg-white rounded-xl p-5 border border-slate-100">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-2xl font-bold text-red-600">{validationData.errorRows}</p><p className="text-xs text-[#464555]">Erreurs</p></div>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowErrorsOnly(!showErrorsOnly)}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${showErrorsOnly ? 'bg-red-100 text-red-700' : 'bg-[#f3f4f5] text-[#464555] hover:bg-slate-200'}`}>
                <Eye size={14} /> {showErrorsOnly ? 'Tout afficher' : 'Erreurs uniquement'}
              </button>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-[#f8f9fa] sticky top-0">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-bold text-[#464555] uppercase">Email</th>
                      <th className="px-4 py-3 text-left text-xs font-bold text-[#464555] uppercase">Matière</th>
                    ...
                    {displayRows.map((row, i) => (
                      <tr key={i} className={`border-t border-slate-50 ${row.valid ? '' : 'bg-red-50/50'}`}>
                        ...
                        <td className="px-4 py-3">
                          {row.valid ? (
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-bold"><Check size={12} /> OK</span>
                          ) : (
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-red-100 text-red-700 rounded-full text-xs font-bold"><X size={12} /> Erreur</span>
                          )}
                        </td>
                        <td className="px-4 py-3 font-medium text-[#191c1d]">{row.nom}</td>
                        <td className="px-4 py-3 text-[#191c1d]">{row.prenom}</td>
                        <td className="px-4 py-3 text-[#464555] font-mono text-xs">{row.email}</td>
                        <td className="px-4 py-3 text-[#464555]">{row.matierePrincipale}</td>
                        <td className="px-4 py-3">
                          {row.errors.length > 0 && (
                            <div className="space-y-1">{row.errors.map((err, j) => (<p key={j} className="text-xs text-red-600">• {err}</p>))}</div>
                          )}
                        </td>
                      </tr>
                    ))}
            ...
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setStep('confirm')} disabled={validationData.validRows === 0}
                className="px-8 py-3 bg-gradient-to-r from-[#3525cd] to-[#4f46e5] text-white font-bold rounded-xl shadow-lg flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-xl transition-all">
                Confirmer {validationData.validRows} professeurs <ArrowRight size={18} />
              </button>
            </div>
          </div>
        )}
        ...
        {step === 'confirm' && validationData && (
          <div className="bg-white rounded-2xl p-10 border border-slate-100 text-center">
            ...
            <p className="text-[#464555] mb-2 max-w-lg mx-auto">
              Vous êtes sur le point de créer <strong className="text-[#191c1d]">{validationData.validRows} comptes professeurs</strong>.
            </p>
            {validationData.errorRows > 0 && (
              <p className="text-sm text-red-600 mb-6">{validationData.errorRows} ligne(s) avec erreurs seront ignorées.</p>
            )}
            <div className="bg-[#f8f9fa] rounded-xl p-6 mb-8 max-w-md mx-auto text-left">
              ...
              <ul className="space-y-2 text-sm text-[#464555]">
                <li className="flex items-center gap-2"><Check size={14} className="text-emerald-500" /> {validationData.validRows} comptes professeurs</li>
                <li className="flex items-center gap-2"><Check size={14} className="text-emerald-500" /> Association aux matières</li>
                <li className="flex items-center gap-2"><Check size={14} className="text-emerald-500" /> Association aux classes</li>
            ...
            <div className="flex items-center justify-center gap-4">
              <button onClick={() => setStep('validate')} className="px-6 py-3 text-[#464555] font-medium hover:bg-slate-100 rounded-xl transition-colors">Retour</button>
        ...
        {step === 'result' && importResult && (
          <div>
            <div className="bg-white rounded-2xl p-10 border border-slate-100 text-center mb-8">
              ...
              <p className="text-[#464555] mb-8">Les comptes professeurs ont été créés avec succès.</p>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-sm mx-auto mb-8">
                <div className="bg-blue-50 rounded-xl p-4">
                  <p className="text-2xl font-bold text-blue-600">{importResult.teachersCreated}</p>
                  <p className="text-xs text-blue-700">Professeurs créés</p>
                </div>
            ...
            {importResult.credentials.length > 0 && (
              <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden mb-8">
                <div className="px-6 py-4 border-b border-slate-100">
                  ...
                  <p className="text-xs text-[#464555]">Distribuez ces accès aux professeurs</p>
                </div>
                <div className="overflow-x-auto max-h-[300px] overflow-y-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-[#f8f9fa] sticky top-0">
                      <tr>
                        <th className="px-4 py-3 text-left text-xs font-bold text-[#464555] uppercase">Professeur</th>
                        <th className="px-4 py-3 text-left text-xs font-bold text-[#464555] uppercase">Matière</th>
                        <th className="px-4 py-3 text-left text-xs font-bold text-[#464555] uppercase">Identifiant</th>
                    ...
                    <tbody>
                      {importResult.credentials.map((cred, i) => (
                        <tr key={i} className="border-t border-slate-50">
                          <td className="px-4 py-3 font-medium text-[#191c1d]">{cred.teacherName}</td>
                          <td className="px-4 py-3 text-[#464555]">{cred.subject}</td>
                          <td className="px-4 py-3 font-mono text-xs text-[#191c1d]">{cred.login}</td>
                          <td className="px-4 py-3 font-mono text-xs font-bold text-red-600">{cred.password}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
