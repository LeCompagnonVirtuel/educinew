...
export default function ApiHubPage() {
  const { data, loading, error, refresh } = useMobileApi<ApiHubItem>({
    endpoint: '/api/interoperability/apis',
  });
        ...
        {entities.map((entity, i) => (
          <div key={entity.id ?? i} className="flex items-center justify-between p-3 border-b last:border-0">
            <div>
              ...
              <div className="text-xs text-gray-500">{entity.version ?? ''} {entity.calls ? `• ${entity.calls} calls` : ''}</div>
            </div>
