  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 animate-pulse">
      ...
      <div className="bg-white rounded-lg shadow-sm">
        <div className="h-4 w-32 bg-gray-200 rounded m-3" />
        {[...Array(3)].map((_, i) => (
...
export default function CredentialsWalletPage() {
  const { data, loading, error, refresh } = useMobileApi<CredentialItem>({
    endpoint: '/api/interoperability/credentials',
  });
        ...
        {entities.map((entity, i) => (
          <div key={entity.id ?? i} className="flex items-center justify-between p-3 border-b last:border-0">
            <div>
              ...
              <div className="text-xs text-gray-500">{entity.issuer ?? ''} {entity.date ? `• ${entity.date}` : ''}</div>
            </div>
            <span className={`px-2 py-1 text-xs rounded-full ${
              entity.status === 'active' || entity.status === 'verified'
