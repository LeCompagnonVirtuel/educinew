  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 animate-pulse">
      ...
      <div className="bg-white rounded-lg shadow-sm">
        <div className="h-4 w-36 bg-gray-200 rounded m-3" />
        {[...Array(3)].map((_, i) => (
        ...
        {entities.map((entity, i) => (
          <div key={entity.id ?? i} className="flex items-center justify-between p-3 border-b last:border-0">
            <div>
              ...
              <div className="text-xs text-gray-500">{entity.developer ?? ''} {entity.category ? `• ${entity.category}` : ''}</div>
            </div>
            <span className={`px-2 py-1 text-xs rounded-full ${
              entity.installed || entity.status === 'installed' || entity.status === 'active'
                ? 'bg-green-100 text-green-700'
                : 'bg-gray-100 text-gray-700'
            }`}>
              {entity.installed ? 'Installed' : entity.status ?? 'Available'}
            </span>
          </div>
        ))}
      </div>
