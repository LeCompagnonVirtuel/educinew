...
export default function DataMeshPage() {
  const { data, loading, error, refresh } = useMobileApi<DataMeshItem>({
    endpoint: '/api/interoperability/data-products',
  });
        ...
        {entities.map((entity, i) => (
          <div key={entity.id ?? i} className="flex items-center justify-between p-3 border-b last:border-0">
            <div>
              ...
              <div className="text-xs text-gray-500">{entity.owner ?? ''} {entity.products ? `• ${entity.products} products` : ''}</div>
            </div>
            <span className="px-2 py-1 text-xs rounded-full bg-green-100 text-green-700">
              {entity.quality ?? entity.status ?? 'OK'}
            </span>
          </div>
        ))}
      </div>
