...
export default function IntegrationsPage() {
  const { data, loading, error, refresh } = useMobileApi<ConnectorItem>({
    endpoint: '/api/interoperability/connectors',
  });
        ...
        {entities.map((entity, i) => (
          <div key={entity.id ?? i} className="flex items-center justify-between p-3 border-b last:border-0">
            <div>
              ...
              <div className="text-xs text-gray-500">{entity.type ?? ''} {entity.lastSync ? `• Last sync: ${entity.lastSync}` : ''}</div>
            </div>
            <span className={`px-2 py-1 text-xs rounded-full ${
              entity.status === 'active' || entity.status === 'connected'
