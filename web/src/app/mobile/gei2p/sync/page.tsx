...
export default function SyncMonitorPage() {
  const { data, loading, error, refresh } = useMobileApi<SyncItem>({
    endpoint: '/api/interoperability/sync-jobs',
  });
        ...
        {entities.map((entity, i) => (
          <div key={entity.id ?? i} className="flex items-center justify-between p-3 border-b last:border-0">
            <div>
              ...
              <div className="text-xs text-gray-500">{entity.direction ?? ''} {entity.lastSync ? `• ${entity.lastSync}` : ''}</div>
            </div>
            <span className={`px-2 py-1 text-xs rounded-full ${
              entity.status === 'success' || entity.status === 'active' || entity.status === 'synced'
