...
interface AutonomousAcademicItem {
  name?: string;
  title?: string;
  label?: string;
  count?: number;
...
export default function AutonomousAcademicPage() {
  const { data, loading, error, refresh } = useMobileApi<AutonomousAcademicItem>({
    endpoint: '/api/lxp/courses',
    transform: (items) =>
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="mb-6 flex items-center justify-between">
        ...
        <button
          onClick={() => refresh()}
          disabled={loading}
          className="rounded-lg bg-orange-600 px-4 py-2 text-sm font-medium text-white active:bg-orange-700 disabled:opacity-50"
        >
      ...
      <div className="mb-6 grid grid-cols-3 gap-3">
        {stats.map((s) => (
          <div key={s.label} className="rounded-xl bg-white p-3 shadow-sm">
            <p className="text-lg font-bold text-orange-600">{String(s.value)}</p>
