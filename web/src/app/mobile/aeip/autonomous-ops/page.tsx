...
export default function AutonomousOpsPage() {
  const { data, loading, error, refresh } = useMobileApi<AutonomousOpsItem>({
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-900">Autonomous Operations</h1>
        <button
          onClick={() => refresh()}
          disabled={loading}
          className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white active:bg-emerald-700 disabled:opacity-50"
        >
      ...
      <div className="mb-6 grid grid-cols-3 gap-3">
        {stats.map((s) => (
          <div key={s.label} className="rounded-xl bg-white p-3 shadow-sm">
            <p className="text-lg font-bold text-emerald-600">{String(s.value)}</p>
