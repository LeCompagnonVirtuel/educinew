...
interface GenerativeStudioItem {
  name?: string;
  title?: string;
  label?: string;
  count?: number;
...
export default function GenerativeStudioPage() {
  const { data, loading, error, refresh } = useMobileApi<GenerativeStudioItem>({
    endpoint: '/api/integration/ai/prompts',
    transform: (items) =>
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="mb-6 flex items-center justify-between">
        ...
        <button
          onClick={() => refresh()}
          disabled={loading}
          className="rounded-lg bg-cyan-600 px-4 py-2 text-sm font-medium text-white active:bg-cyan-700 disabled:opacity-50"
        >
      ...
      <div className="mb-6 grid grid-cols-3 gap-3">
        {stats.map((s) => (
          <div key={s.label} className="rounded-xl bg-white p-3 shadow-sm">
            <p className="text-lg font-bold text-cyan-600">{String(s.value)}</p>
