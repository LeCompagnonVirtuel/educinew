...
export default function CertificationsPage() {
  const { data, loading, refresh } = useMobileApi({
    endpoint: '/api/lifelong-learning/certifications',
  });
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="mb-6 flex items-center justify-between">
        ...
        <button
          onClick={refresh}
          disabled={loading}
          className="rounded-lg bg-amber-600 px-4 py-2 text-sm font-medium text-white active:bg-amber-700 disabled:opacity-50"
        >
        ...
        {loading
          ...
          : stats.map((s) => (
              <div key={s.label} className="rounded-xl bg-white p-3 shadow-sm">
                <p className="text-lg font-bold text-amber-600">{String(s.value)}</p>
      ...
      <div className="rounded-xl bg-white p-4 shadow-sm">
        <h2 className="mb-3 text-sm font-semibold text-gray-700">Certification Entities</h2>
