...
export default function WellbeingAssessmentPage() {
  const [stats] = useState({
    totalAssessments: 1100,
    completedAssessments: 950,
    pendingAssessments: 150,
    averageScore: 76,
    highRiskStudents: 22,
    followUpsScheduled: 38,
  });
  ...
  const entities = [
    { name: 'Total Assessments', count: stats.totalAssessments, icon: '📝' },
    { name: 'Completed', count: stats.completedAssessments, icon: '✅' },
    { name: 'Pending', count: stats.pendingAssessments, icon: '⏳' },
    { name: 'Average Score', count: stats.averageScore, icon: '📊' },
    { name: 'High Risk', count: stats.highRiskStudents, icon: '🔴' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Wellbeing Assessment</h1>
          <p className="text-sm text-gray-500">Assessment & Evaluation</p>
        </div>
        <button
