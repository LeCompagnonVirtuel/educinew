...
const ENTITIES = [
  { id: '1', name: 'StudentIdentity', description: 'Student digital identity and credentials', status: 'verified', count: 2450 },
  { id: '2', name: 'TeacherIdentity', description: 'Teacher professional identity management', status: 'verified', count: 180 },
  { id: '3', name: 'SchoolIdentity', description: 'School institutional identity and reputation', status: 'active', count: 12 },
];
  ...
  return (
    <div className="flex-1 bg-gray-50 min-h-screen">
      <div className="px-4 pt-4 pb-3">
        <h1 className="text-2xl font-extrabold text-gray-900">Digital Identity</h1>
        <p className="text-sm text-gray-500 mt-1">StudentIdentity · TeacherIdentity · SchoolIdentity</p>
      </div>
      ...
      <div className="px-4 space-y-3">
        {data.map((item) => (
          <div key={item.id} className="bg-white rounded-xl p-4 shadow-sm">
            ...
            <p className="text-xs text-blue-600 mt-2 font-semibold">{item.count.toLocaleString()} identities</p>
          </div>
        ))}
      </div>
    </div>
