...
const ENTITIES = [
  { id: '1', name: 'RuntimeManager', description: 'Core runtime orchestration engine', status: 'active', count: 12 },
  { id: '2', name: 'ModuleRegistry', description: 'Dynamic module registration system', status: 'active', count: 24 },
  { id: '3', name: 'ServiceRegistry', description: 'Microservice discovery and routing', status: 'active', count: 18 },
];
  ...
  return (
    <div className="flex-1 bg-gray-50 min-h-screen">
      <div className="px-4 pt-4 pb-3">
        <h1 className="text-2xl font-extrabold text-gray-900">Core Runtime</h1>
        <p className="text-sm text-gray-500 mt-1">RuntimeManager · ModuleRegistry · ServiceRegistry</p>
      </div>
      <div className="flex mx-4 mb-4 bg-white rounded-xl p-4 shadow-sm">
        ...
        <div className="flex-1 text-center">
          ...
          <div className="text-xs text-gray-500 mt-1">Instances</div>
        </div>
        <div className="flex-1 text-center">
      ...
      <div className="px-4 space-y-3">
        {data.map((item) => (
          <div key={item.id} className="bg-white rounded-xl p-4 shadow-sm">
            ...
            <p className="text-xs text-blue-600 mt-2 font-semibold">{item.count} instances</p>
          </div>
        ))}
      </div>
    </div>
