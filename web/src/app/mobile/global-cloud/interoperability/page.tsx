...
interface InteroperabilityItem {
  name?: string;
  title?: string;
  label?: string;
  count?: number;
...
export default function InteroperabilityPage() {
  const { data, loading, error, refresh } = useMobileApi<InteroperabilityItem>({
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-gray-900">International Interoperability</h1>
        <button
          onClick={() => refresh()}
          disabled={loading}
