...
export default function EmploymentPage() {
  const { data, loading, error, refresh } = useMobileApi<EmploymentItem>({
    endpoint: '/api/employment/jobs',
  })

  const stats = [
    { label: 'Total', value: data.length },
    { label: 'Filled', value: data.filter((d) => d.status === 'Filled').length },
    { label: 'Vacant', value: data.filter((d) => d.status !== 'Filled').length },
  ]
