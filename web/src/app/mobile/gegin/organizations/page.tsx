...
export default function OrganizationsPage() {
  const { data, loading, error, refresh } = useMobileApi<OrganizationItem>({
    endpoint: '/api/schools/schools',
  })

  const stats = [
    { label: 'Total', value: data.length },
    { label: 'Partners', value: data.filter((d) => d.status === 'Partner').length },
    { label: 'Other', value: data.filter((d) => d.status !== 'Partner').length },
  ]
