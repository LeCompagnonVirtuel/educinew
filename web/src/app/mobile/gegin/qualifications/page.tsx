...
export default function QualificationsPage() {
  const { data, loading, error, refresh } = useMobileApi<QualificationItem>({
    endpoint: '/api/interoperability/qualifications',
  })

  const stats = [
    { label: 'Total', value: data.length },
    { label: 'Certified', value: data.filter((d) => d.status === 'Certified').length },
    { label: 'Pending', value: data.filter((d) => d.status !== 'Certified').length },
  ]
