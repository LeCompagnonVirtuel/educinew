...
export default function DigitalTwinPage() {
  const { data, loading, error, refresh } = useMobileApi<DigitalTwinItem>({
    endpoint: '/api/digital-twin/twins/twins',
  })

  const stats = [
    { label: 'Total', value: data.length },
    { label: 'Synced', value: data.filter((d) => d.status === 'Synced').length },
    { label: 'Issues', value: data.filter((d) => d.status !== 'Synced').length },
  ]
