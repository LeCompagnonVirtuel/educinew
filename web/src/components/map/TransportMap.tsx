...
const createIcon = (L: typeof import('leaflet').default, color: string, icon: string) => L.divIcon({
...
export default function TransportMap({
  ...
  useEffect(() => {
    ...
    (async () => {
      const L = (await import('leaflet')).default;
      const schoolIcon = createIcon(L, '#3525cd', '🏫');
      const busIcon = createIcon(L, '#F59E0B', '🚌');
      const stopIcon = createIcon(L, '#22C55E', '🚏');
      const parentIcon = createIcon(L, '#EC4899', '📍');
