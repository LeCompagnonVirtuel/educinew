...
export const GovInspectionReportDetailScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/inspection-report-detail');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Détails du Rapport</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovInspectionReportDetailInfo', { id: item.id })}>
