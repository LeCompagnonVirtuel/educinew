...
export const GovCountryListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/countries');
    ...
    <ScrollView style={styles.container}>
      ...
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovCountryDetail', { id: item.id })}>
