...
export const GovQualityAuditListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/quality-audits');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Audits Qualité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovQualityAuditDetail', { id: item.id })}>
