...
export const GovDirectorateListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/directorates');
    ...
    <ScrollView style={styles.container}>
      ...
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovDirectorateDetail', { id: item.id })}>
