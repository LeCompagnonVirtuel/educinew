...
export const GovCertificateListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/certificates');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Certificats</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovCertificateDetail', { id: item.id })}>
