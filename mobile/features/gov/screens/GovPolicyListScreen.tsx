...
export const GovPolicyListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/policies');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Politiques</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovPolicyDetail', { id: item.id })}>
