...
export const GovTeacherDistributionScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/teacher-distribution');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Distribution des Enseignants</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovTeacherDistributionDetail', { id: item.id })}>
