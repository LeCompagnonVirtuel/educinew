...
export const GovCampusAnalyticsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/campus-analytics');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Analytiques Campus</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovCampusAnalyticsDetail', { id: item.id })}>
