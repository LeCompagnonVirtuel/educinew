...
export const GovBudgetAnalyticScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/budget-analytics');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Analytiques Budgétaires</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovBudgetAnalyticDetail', { id: item.id })}>
