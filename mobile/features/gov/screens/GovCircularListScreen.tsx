...
export const GovCircularListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/circulars');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Circulaires</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovCircularDetail', { id: item.id })}>
