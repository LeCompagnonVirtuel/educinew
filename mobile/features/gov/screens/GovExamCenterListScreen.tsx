...
export const GovExamCenterListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/exam-centers');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Centres d'Examen</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovExamCenterDetail', { id: item.id })}>
