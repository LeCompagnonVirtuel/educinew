...
export const GovInspectionMissionListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/inspection-missions');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Missions d'Inspection</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovInspectionMissionDetail', { id: item.id })}>
