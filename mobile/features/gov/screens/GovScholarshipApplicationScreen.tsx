...
export const GovScholarshipApplicationScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/scholarship-applications');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Demandes de Bourse</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovScholarshipApplicationDetail', { id: item.id })}>
