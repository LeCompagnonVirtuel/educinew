...
export const GovMinistryListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/ministries');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Liste des Ministères</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovMinistryDetail', { id: item.id })}>
