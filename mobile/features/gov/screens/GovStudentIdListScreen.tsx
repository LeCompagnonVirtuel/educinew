...
export const GovStudentIdListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/student-ids');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>IDs Étudiants Nationaux</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovStudentIdDetail', { id: item.id })}>
