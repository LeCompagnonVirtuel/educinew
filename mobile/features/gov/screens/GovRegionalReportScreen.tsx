...
export const GovRegionalReportScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/regions/reports');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Rapports Régionaux</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovRegionalReportDetail', { id: item.id })}>
