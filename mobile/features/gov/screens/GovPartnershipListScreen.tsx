...
export const GovPartnershipListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/partnerships');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Partenariats Internationaux</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovPartnershipDetail', { id: item.id })}>
