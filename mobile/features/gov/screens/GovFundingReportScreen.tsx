...
export const GovFundingReportScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/funding-reports');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Rapports de Financement</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovFundingReportDetail', { id: item.id })}>
