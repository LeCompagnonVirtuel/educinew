...
export const GovKpiListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/kpis');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>KPIs Éducation</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovKpiDetail', { id: item.id })}>
