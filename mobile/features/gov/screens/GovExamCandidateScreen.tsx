...
export const GovExamCandidateScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/exam-candidates');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Candidats aux Examens</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovExamCandidateDetail', { id: item.id })}>
