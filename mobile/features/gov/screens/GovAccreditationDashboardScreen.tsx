...
export const GovAccreditationDashboardScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/accreditations/dashboard');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Tableau de bord Accréditation</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovAccreditationDashboardDetail', { id: item.id })}>
