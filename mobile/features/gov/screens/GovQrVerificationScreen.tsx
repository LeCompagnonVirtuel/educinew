...
export const GovQrVerificationScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/qr-verification');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Vérification QR</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovQrVerificationDetail', { id: item.id })}>
