...
export const GovCampusListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/campuses');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Liste des Campus</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovCampusDetail', { id: item.id })}>
