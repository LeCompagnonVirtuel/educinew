...
export const GovTeacherRegistryScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/teacher-registry');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Registre des Enseignants</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovTeacherRegistryDetail', { id: item.id })}>
