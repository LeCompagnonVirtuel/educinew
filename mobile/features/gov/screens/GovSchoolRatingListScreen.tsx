...
export const GovSchoolRatingListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/school-ratings');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Classements des Écoles</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovSchoolRatingDetail', { id: item.id })}>
