...
export const GovExamListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/exams');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Examens Nationaux</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovExamDetail', { id: item.id })}>
