...
export const GovCampusTransferScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/campus-transfers');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Transferts Inter-campus</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovCampusTransferDetail', { id: item.id })}>
