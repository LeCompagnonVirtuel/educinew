...
export const GovQualityIndicatorScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/quality-indicators');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Indicateurs Qualité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovQualityIndicatorDetail', { id: item.id })}>
