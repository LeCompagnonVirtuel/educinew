...
export const GovAccreditationListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/accreditations');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Liste des Accréditations</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovAccreditationDetail', { id: item.id })}>
