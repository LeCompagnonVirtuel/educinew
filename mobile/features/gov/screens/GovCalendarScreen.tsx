...
export const GovCalendarScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/calendar');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Calendrier Éducatif</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovCalendarDetail', { id: item.id })}>
