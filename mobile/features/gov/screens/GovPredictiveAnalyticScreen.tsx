...
export const GovPredictiveAnalyticScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/predictive-analytics');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Analytiques Prédictives</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovPredictiveAnalyticDetail', { id: item.id })}>
