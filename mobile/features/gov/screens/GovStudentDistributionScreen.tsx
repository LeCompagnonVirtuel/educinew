...
export const GovStudentDistributionScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/student-distribution');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Distribution des Élèves</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovStudentDistributionDetail', { id: item.id })}>
