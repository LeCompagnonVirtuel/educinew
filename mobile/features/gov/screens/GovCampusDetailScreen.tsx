...
export const GovCampusDetailScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/campus-detail');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Détails du Campus</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovCampusDetailInfo', { id: item.id })}>
