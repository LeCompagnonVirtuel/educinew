...
export const GovExamResultScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/exam-results');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Résultats d'Examen</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovExamResultDetail', { id: item.id })}>
