...
export const GovComplianceOverviewScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/compliance-overview');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Vue d'Ensemble Conformité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovComplianceOverviewDetail', { id: item.id })}>
