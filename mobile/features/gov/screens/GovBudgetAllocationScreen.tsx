...
export const GovBudgetAllocationScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/budget-allocations');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Allocations Budgétaires</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovBudgetAllocationDetail', { id: item.id })}>
