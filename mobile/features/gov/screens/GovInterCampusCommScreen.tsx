...
export const GovInterCampusCommScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/inter-campus-comms');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Communications Inter-campus</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovInterCampusCommDetail', { id: item.id })}>
