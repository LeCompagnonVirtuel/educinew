...
export const GovInfrastructureMapScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/infrastructure-maps');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Carte des Infrastructures</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovInfrastructureMapDetail', { id: item.id })}>
