...
export const GovNetworkListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/networks');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Liste des Réseaux</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovNetworkDetail', { id: item.id })}>
