...
export const GovNetworkDetailScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/network-detail');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Détails du Réseau</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovNetworkDetailInfo', { id: item.id })}>
