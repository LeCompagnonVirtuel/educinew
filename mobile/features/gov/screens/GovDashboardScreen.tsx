...
export const GovDashboardScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/dashboard');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Tableau de bord Ministère</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovDashboardDetail', { id: item.id })}>
