...
export const GovFundingListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/funding');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Financement Gouvernemental</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovFundingDetail', { id: item.id })}>
