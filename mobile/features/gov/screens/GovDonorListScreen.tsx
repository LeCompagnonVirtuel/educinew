...
export const GovDonorListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/donors');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Donateurs</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovDonorDetail', { id: item.id })}>
