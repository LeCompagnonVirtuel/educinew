...
export const GovInspectionMissionDetailScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/inspection-mission-detail');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Détails de la Mission</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovInspectionMissionDetailInfo', { id: item.id })}>
