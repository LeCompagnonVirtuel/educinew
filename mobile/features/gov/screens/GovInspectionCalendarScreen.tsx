...
export const GovInspectionCalendarScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/inspection-calendar');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Calendrier d'Inspection</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovInspectionCalendarDetail', { id: item.id })}>
