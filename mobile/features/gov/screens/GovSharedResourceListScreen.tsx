...
export const GovSharedResourceListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/shared-resources');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Ressources Partagées</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovSharedResourceDetail', { id: item.id })}>
