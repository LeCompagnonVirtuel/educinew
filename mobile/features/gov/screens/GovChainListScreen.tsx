...
export const GovChainListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/chains');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Chaînes Scolaires</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovChainDetail', { id: item.id })}>
