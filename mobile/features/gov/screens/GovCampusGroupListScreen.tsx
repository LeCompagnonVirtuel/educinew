...
export const GovCampusGroupListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/campus-groups');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Groupes de Campus</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovCampusGroupDetail', { id: item.id })}>
