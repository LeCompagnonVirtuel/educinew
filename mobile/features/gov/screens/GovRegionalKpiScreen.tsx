...
export const GovRegionalKpiScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/regions/kpis');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>KPIs Régionaux</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovRegionalKpiDetail', { id: item.id })}>
