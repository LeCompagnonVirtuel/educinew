...
export const GovScholarshipListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/scholarships');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Bourses d'Études</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovScholarshipDetail', { id: item.id })}>
