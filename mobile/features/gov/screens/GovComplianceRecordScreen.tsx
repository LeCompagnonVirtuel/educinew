...
export const GovComplianceRecordScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/compliance-records');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Registres de Conformité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovComplianceRecordDetail', { id: item.id })}>
