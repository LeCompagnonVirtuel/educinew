...
export const GovExamScheduleScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/exam-schedules');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Emploi du temps Examens</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovExamScheduleDetail', { id: item.id })}>
