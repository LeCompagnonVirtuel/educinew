...
export const GovExamFraudScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/exam-fraud');
    ...
    <ScrollView style={styles.container}>
      ...
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovExamFraudDetail', { id: item.id })}>
