...
export const GovNationalStandardListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/national-standards');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Standards Nationaux</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovNationalStandardDetail', { id: item.id })}>
