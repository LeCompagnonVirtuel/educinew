...
export const GovInternationalDashboardScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/international/dashboard');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Tableau de bord International</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovInternationalDashboardDetail', { id: item.id })}>
