...
export const GovRegionListScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/gov/regions');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Liste des Régions</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('GovRegionDetail', { id: item.id })}>
