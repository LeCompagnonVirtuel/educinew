...
export const SearchIndicesScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/search/indices');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Indices de recherche</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SearchIndicesScreenDetail', { id: item.id })}>
