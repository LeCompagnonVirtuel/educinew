...
export const SearchPeopleSearchScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/search/people');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Recherche de personnes</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SearchPeopleSearchScreenDetail', { id: item.id })}>
