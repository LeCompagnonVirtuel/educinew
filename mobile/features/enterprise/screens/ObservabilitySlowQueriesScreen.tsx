...
export const ObservabilitySlowQueriesScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/observability/slow-queries');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Requêtes lentes</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ObservabilitySlowQueriesScreenDetail', { id: item.id })}>
