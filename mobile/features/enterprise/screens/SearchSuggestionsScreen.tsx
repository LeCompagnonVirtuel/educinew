...
export const SearchSuggestionsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/search/suggestions');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Suggestions de recherche</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SearchSuggestionsScreenDetail', { id: item.id })}>
