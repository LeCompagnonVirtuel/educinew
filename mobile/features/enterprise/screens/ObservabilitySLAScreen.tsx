...
export const ObservabilitySLAScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/observability/sla');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Accords de niveau de service</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ObservabilitySLAScreenDetail', { id: item.id })}>
