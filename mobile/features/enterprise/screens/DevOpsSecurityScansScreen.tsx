...
export const DevOpsSecurityScansScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/devops/security-scans');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Analyses de sécurité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DevOpsSecurityScansScreenDetail', { id: item.id })}>
