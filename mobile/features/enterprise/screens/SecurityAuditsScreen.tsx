...
export const SecurityAuditsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/security/audits');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Audits de sécurité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SecurityAuditsScreenDetail', { id: item.id })}>
