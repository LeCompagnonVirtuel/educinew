...
export const TenantBackupScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/tenants/backup');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Sauvegarde des locataires</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('TenantBackupScreenDetail', { id: item.id })}>
