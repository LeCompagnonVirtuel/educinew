...
export const DataSnapshotsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/data/snapshots');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Instantanés de données</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DataSnapshotsScreenDetail', { id: item.id })}>
