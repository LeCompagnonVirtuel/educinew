...
export const DataQualityScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/data/quality');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Qualité des données</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DataQualityScreenDetail', { id: item.id })}>
