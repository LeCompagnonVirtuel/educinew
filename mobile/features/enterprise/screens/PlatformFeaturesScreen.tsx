...
export const PlatformFeaturesScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/platform-features');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Fonctionnalités de la plateforme</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('PlatformFeaturesDetail', { id: item.id })}>
