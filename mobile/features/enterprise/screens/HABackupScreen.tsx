...
export const HABackupScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/ha/backup');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Sauvegarde haute disponibilité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('HABackupScreenDetail', { id: item.id })}>
