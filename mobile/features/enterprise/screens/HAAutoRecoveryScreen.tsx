...
export const HAAutoRecoveryScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/ha/auto-recovery');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Récupération automatique</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('HAAutoRecoveryScreenDetail', { id: item.id })}>
