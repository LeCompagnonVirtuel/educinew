...
export const ObservabilityErrorDashboardScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/observability/errors');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Tableau de bord erreurs</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ObservabilityErrorDashboardScreenDetail', { id: item.id })}>
