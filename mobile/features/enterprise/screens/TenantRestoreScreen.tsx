...
export const TenantRestoreScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/tenants/restore');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Restauration des locataires</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('TenantRestoreScreenDetail', { id: item.id })}>
