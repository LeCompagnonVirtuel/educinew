...
export const DataArchivesScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/data/archives');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Archives de données</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DataArchivesScreenDetail', { id: item.id })}>
