...
export const ObservabilitySLOScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/observability/slo');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Objectifs de niveau de service</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ObservabilitySLOScreenDetail', { id: item.id })}>
