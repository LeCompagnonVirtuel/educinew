...
export const SearchElasticClusterScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/search/elastic-cluster');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Cluster Elastic</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SearchElasticClusterScreenDetail', { id: item.id })}>
