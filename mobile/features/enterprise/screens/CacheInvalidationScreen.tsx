...
export const CacheInvalidationScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/cache/invalidation');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Invalidation de cache</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('CacheInvalidationScreenDetail', { id: item.id })}>
