...
export const SecurityZeroTrustScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/security/zero-trust');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Zéro confiance</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SecurityZeroTrustScreenDetail', { id: item.id })}>
