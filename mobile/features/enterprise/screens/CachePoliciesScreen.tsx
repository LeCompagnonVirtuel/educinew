...
export const CachePoliciesScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/cache/policies');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Politiques de cache</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('CachePoliciesScreenDetail', { id: item.id })}>
