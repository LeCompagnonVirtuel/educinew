...
export const DataPipelinesScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/data/pipelines');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Pipelines de données</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DataPipelinesScreenDetail', { id: item.id })}>
