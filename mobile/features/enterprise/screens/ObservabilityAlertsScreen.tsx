...
export const ObservabilityAlertsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/observability/alerts');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Alertes d'observabilité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ObservabilityAlertsScreenDetail', { id: item.id })}>
