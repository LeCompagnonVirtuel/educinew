...
export const HADisasterRecoveryScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/ha/disaster-recovery');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Récupération après sinistre</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('HADisasterRecoveryScreenDetail', { id: item.id })}>
