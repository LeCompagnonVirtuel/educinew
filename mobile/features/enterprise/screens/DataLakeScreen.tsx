...
export const DataLakeScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/data/lake');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Lac de données</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DataLakeScreenDetail', { id: item.id })}>
