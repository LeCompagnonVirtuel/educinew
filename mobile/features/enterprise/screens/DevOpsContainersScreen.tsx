...
export const DevOpsContainersScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/devops/containers');
    ...
    <ScrollView style={styles.container}>
      ...
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DevOpsContainersScreenDetail', { id: item.id })}>
