...
export const ObservabilityLoggingScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/observability/logging');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Journalisation</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ObservabilityLoggingScreenDetail', { id: item.id })}>
