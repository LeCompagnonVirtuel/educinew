...
export const PlatformMetricsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/platform-metrics');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Métriques de la plateforme</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('PlatformMetricsDetail', { id: item.id })}>
