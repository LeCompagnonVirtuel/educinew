...
export const OpenDeveloperPortalScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/open-platform/developer-portal');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Portail développeur</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('OpenDeveloperPortalScreenDetail', { id: item.id })}>
