...
export const CacheAPIScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/cache/api');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Cache API</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('CacheAPIScreenDetail', { id: item.id })}>
