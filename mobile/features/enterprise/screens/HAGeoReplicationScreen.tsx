...
export const HAGeoReplicationScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/ha/geo-replication');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Réplication géographique</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('HAGeoReplicationScreenDetail', { id: item.id })}>
