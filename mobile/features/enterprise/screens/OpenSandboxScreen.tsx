...
export const OpenSandboxScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/open-platform/sandbox');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Environnement de test</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('OpenSandboxScreenDetail', { id: item.id })}>
