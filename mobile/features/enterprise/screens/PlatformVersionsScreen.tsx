...
export const PlatformVersionsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/platform-versions');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Versions de la plateforme</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('PlatformVersionsDetail', { id: item.id })}>
