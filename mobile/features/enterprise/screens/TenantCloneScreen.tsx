...
export const TenantCloneScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/tenants/clone');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Clonage des locataires</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('TenantCloneScreenDetail', { id: item.id })}>
