...
export const OpenOAuthScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/open-platform/oauth');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>OAuth</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('OpenOAuthScreenDetail', { id: item.id })}>
