...
export const SearchIndexBuilderScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/search/index-builder');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Constructeur d'index</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SearchIndexBuilderScreenDetail', { id: item.id })}>
