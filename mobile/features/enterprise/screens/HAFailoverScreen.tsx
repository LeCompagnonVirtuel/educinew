...
export const HAFailoverScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/ha/failover');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Basculement automatique</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('HAFailoverScreenDetail', { id: item.id })}>
