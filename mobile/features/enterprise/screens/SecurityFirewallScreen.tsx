...
export const SecurityFirewallScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/security/firewall');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Pare-feu</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SecurityFirewallScreenDetail', { id: item.id })}>
