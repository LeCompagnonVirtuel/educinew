...
export const ProdLoadTestsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/production/load-tests');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Tests de charge</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ProdLoadTestsScreenDetail', { id: item.id })}>
