...
export const HAReplicationScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/ha/replication');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Réplication</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('HAReplicationScreenDetail', { id: item.id })}>
