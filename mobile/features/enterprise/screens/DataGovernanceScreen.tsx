...
export const DataGovernanceScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/data/governance');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Gouvernance des données</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DataGovernanceScreenDetail', { id: item.id })}>
