...
export const DevOpsCIPipelinesScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/devops/ci-pipelines');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Pipelines CI</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DevOpsCIPipelinesScreenDetail', { id: item.id })}>
