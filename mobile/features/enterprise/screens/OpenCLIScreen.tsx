...
export const OpenCLIScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/open-platform/cli');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Interface en ligne de commande</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('OpenCLIScreenDetail', { id: item.id })}>
