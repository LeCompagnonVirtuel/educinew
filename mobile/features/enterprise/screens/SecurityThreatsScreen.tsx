...
export const SecurityThreatsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/security/threats');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Menaces de sécurité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SecurityThreatsScreenDetail', { id: item.id })}>
