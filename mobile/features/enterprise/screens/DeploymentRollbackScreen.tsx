...
export const DeploymentRollbackScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/deployments/rollback');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Retour arrière</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DeploymentRollbackScreenDetail', { id: item.id })}>
