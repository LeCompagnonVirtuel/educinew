...
export const PlatformModulesScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/platform-modules');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Modules de la plateforme</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('PlatformModulesDetail', { id: item.id })}>
