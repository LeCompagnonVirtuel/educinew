...
export const ObservabilityMetricsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/observability/metrics');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Métriques d'observabilité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ObservabilityMetricsScreenDetail', { id: item.id })}>
