...
export const DataLineageScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/data/lineage');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Lignée des données</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DataLineageScreenDetail', { id: item.id })}>
