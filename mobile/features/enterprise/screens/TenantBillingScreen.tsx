...
export const TenantBillingScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/tenants/billing');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Facturation des locataires</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('TenantBillingScreenDetail', { id: item.id })}>
