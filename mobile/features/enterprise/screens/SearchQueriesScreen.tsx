...
export const SearchQueriesScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/search/queries');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Requêtes de recherche</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SearchQueriesScreenDetail', { id: item.id })}>
