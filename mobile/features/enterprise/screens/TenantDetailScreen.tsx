...
export const TenantDetailScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/tenants/detail');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Détail du locataire</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('TenantDetailScreenDetail', { id: item.id })}>
