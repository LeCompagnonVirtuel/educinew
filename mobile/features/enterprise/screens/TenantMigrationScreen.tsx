...
export const TenantMigrationScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/tenants/migration');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Migration des locataires</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('TenantMigrationScreenDetail', { id: item.id })}>
