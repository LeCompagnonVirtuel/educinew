...
export const CacheRedisClustersScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/cache/redis-clusters');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Clusters Redis</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('CacheRedisClustersScreenDetail', { id: item.id })}>
