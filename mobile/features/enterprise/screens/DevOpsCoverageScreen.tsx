...
export const DevOpsCoverageScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/devops/coverage');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Couverture de code</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DevOpsCoverageScreenDetail', { id: item.id })}>
