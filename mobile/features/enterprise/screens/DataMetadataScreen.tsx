...
export const DataMetadataScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/data/metadata');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Métadonnées</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DataMetadataScreenDetail', { id: item.id })}>
