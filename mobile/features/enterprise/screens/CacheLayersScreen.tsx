...
export const CacheLayersScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/cache/layers');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Couches de cache</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('CacheLayersScreenDetail', { id: item.id })}>
