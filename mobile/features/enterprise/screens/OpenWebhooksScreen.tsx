...
export const OpenWebhooksScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/open-platform/webhooks');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Webhooks ouverts</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('OpenWebhooksScreenDetail', { id: item.id })}>
