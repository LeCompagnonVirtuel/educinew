...
export const ProdGoLiveScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/production/go-live');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Mise en production</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ProdGoLiveScreenDetail', { id: item.id })}>
