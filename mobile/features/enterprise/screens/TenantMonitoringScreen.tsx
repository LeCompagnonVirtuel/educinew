...
export const TenantMonitoringScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/tenants/monitoring');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Surveillance des locataires</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('TenantMonitoringScreenDetail', { id: item.id })}>
