...
export const SecuritySecretRotationScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/security/secret-rotation');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Rotation des secrets</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SecuritySecretRotationScreenDetail', { id: item.id })}>
