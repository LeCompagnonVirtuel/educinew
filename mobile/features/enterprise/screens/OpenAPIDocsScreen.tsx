...
export const OpenAPIDocsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/open-platform/api-docs');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Documentation API</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('OpenAPIDocsScreenDetail', { id: item.id })}>
