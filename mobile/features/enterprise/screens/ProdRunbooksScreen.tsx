...
export const ProdRunbooksScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/production/runbooks');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Manuels opératoires</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ProdRunbooksScreenDetail', { id: item.id })}>
