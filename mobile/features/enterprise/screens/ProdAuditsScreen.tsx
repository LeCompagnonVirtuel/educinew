...
export const ProdAuditsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/production/audits');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Audits de production</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ProdAuditsScreenDetail', { id: item.id })}>
