...
export const ProdPerformanceBenchmarksScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/production/performance-benchmarks');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Benchmarks de performance</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ProdPerformanceBenchmarksScreenDetail', { id: item.id })}>
