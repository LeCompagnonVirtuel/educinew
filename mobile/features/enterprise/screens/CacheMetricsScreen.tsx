...
export const CacheMetricsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/cache/metrics');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Métriques de cache</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('CacheMetricsScreenDetail', { id: item.id })}>
