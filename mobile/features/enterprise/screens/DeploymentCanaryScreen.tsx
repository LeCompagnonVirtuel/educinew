...
export const DeploymentCanaryScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/deployments/canary');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Déploiement Canary</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DeploymentCanaryScreenDetail', { id: item.id })}>
