...
export const PlatformOverviewScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/platform-overviews');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Vue d'ensemble de la plateforme</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('PlatformOverviewDetail', { id: item.id })}>
