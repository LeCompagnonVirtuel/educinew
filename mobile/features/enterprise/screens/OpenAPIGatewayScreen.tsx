...
export const OpenAPIGatewayScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/open-platform/api-gateway');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Passerelle API</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('OpenAPIGatewayScreenDetail', { id: item.id })}>
