...
export const ProdSecurityBenchmarksScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/production/security-benchmarks');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Benchmarks de sécurité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ProdSecurityBenchmarksScreenDetail', { id: item.id })}>
