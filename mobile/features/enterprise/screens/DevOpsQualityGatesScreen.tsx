...
export const DevOpsQualityGatesScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/devops/quality-gates');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Portes de qualité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DevOpsQualityGatesScreenDetail', { id: item.id })}>
