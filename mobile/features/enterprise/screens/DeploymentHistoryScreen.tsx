...
export const DeploymentHistoryScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/deployments/history');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Historique des déploiements</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DeploymentHistoryScreenDetail', { id: item.id })}>
