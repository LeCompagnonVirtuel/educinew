...
export const OpenSDKScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/open-platform/sdk');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Kit de développement logiciel</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('OpenSDKScreenDetail', { id: item.id })}>
