...
export const ObservabilityServiceHealthScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/observability/health');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Santé des services</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ObservabilityServiceHealthScreenDetail', { id: item.id })}>
