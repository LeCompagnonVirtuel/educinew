...
export const PlatformBackupsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/platform-backups');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Sauvegardes de la plateforme</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('PlatformBackupsScreenDetail', { id: item.id })}>
