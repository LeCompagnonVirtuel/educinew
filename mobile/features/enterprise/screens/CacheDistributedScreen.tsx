...
export const CacheDistributedScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/cache/distributed');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Cache distribué</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('CacheDistributedScreenDetail', { id: item.id })}>
