...
export const PlatformWebhooksScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/platform-webhooks');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Webhooks de la plateforme</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('PlatformWebhooksScreenDetail', { id: item.id })}>
