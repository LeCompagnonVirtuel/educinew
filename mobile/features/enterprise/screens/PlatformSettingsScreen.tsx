...
export const PlatformSettingsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/platform-settings');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Paramètres de la plateforme</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('PlatformSettingsDetail', { id: item.id })}>
