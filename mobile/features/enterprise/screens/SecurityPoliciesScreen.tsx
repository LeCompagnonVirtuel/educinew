...
export const SecurityPoliciesScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/security/policies');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Politiques de sécurité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SecurityPoliciesScreenDetail', { id: item.id })}>
