...
export const DeploymentMigrationScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/deployments/migration');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Migration de déploiement</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DeploymentMigrationScreenDetail', { id: item.id })}>
