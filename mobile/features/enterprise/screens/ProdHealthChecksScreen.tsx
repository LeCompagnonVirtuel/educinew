...
export const ProdHealthChecksScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/production/health-checks');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Vérifications de santé</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ProdHealthChecksScreenDetail', { id: item.id })}>
