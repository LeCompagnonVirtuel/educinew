...
export const HALoadBalancerScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/ha/load-balancer');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Équilibreur de charge</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('HALoadBalancerScreenDetail', { id: item.id })}>
