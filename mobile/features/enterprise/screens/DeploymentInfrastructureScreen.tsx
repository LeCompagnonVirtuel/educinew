...
export const DeploymentInfrastructureScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/deployments/infrastructure');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Infrastructure de déploiement</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DeploymentInfrastructureScreenDetail', { id: item.id })}>
