...
export const DeploymentPromotionScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/deployments/promotion');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Promotion de déploiement</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DeploymentPromotionScreenDetail', { id: item.id })}>
