...
export const CacheQueryScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/cache/query');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Cache de requêtes</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('CacheQueryScreenDetail', { id: item.id })}>
