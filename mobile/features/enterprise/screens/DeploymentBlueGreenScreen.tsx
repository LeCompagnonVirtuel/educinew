...
export const DeploymentBlueGreenScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/deployments/blue-green');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Déploiement Bleu-Vert</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DeploymentBlueGreenScreenDetail', { id: item.id })}>
