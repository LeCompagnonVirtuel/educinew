...
export const TenantIsolationScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/tenants/isolation');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Isolation des locataires</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('TenantIsolationScreenDetail', { id: item.id })}>
