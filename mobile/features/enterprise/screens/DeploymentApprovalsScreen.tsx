...
export const DeploymentApprovalsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/deployments/approvals');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Approbations de déploiement</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DeploymentApprovalsScreenDetail', { id: item.id })}>
