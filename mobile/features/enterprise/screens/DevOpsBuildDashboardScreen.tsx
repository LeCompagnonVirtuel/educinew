...
export const DevOpsBuildDashboardScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/devops/build-dashboard');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Tableau de bord de build</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('DevOpsBuildDashboardScreenDetail', { id: item.id })}>
