...
export const SearchDocumentsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/search/documents');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Documents de recherche</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SearchDocumentsScreenDetail', { id: item.id })}>
