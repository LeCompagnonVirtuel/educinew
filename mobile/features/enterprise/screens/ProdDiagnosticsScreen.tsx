...
export const ProdDiagnosticsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/production/diagnostics');
    ...
    <ScrollView style={styles.container}>
      ...
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ProdDiagnosticsScreenDetail', { id: item.id })}>
