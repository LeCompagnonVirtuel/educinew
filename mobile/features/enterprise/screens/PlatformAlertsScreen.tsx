...
export const PlatformAlertsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/platform-alerts');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Alertes de la plateforme</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('PlatformAlertsScreenDetail', { id: item.id })}>
