...
export const ProdScalabilityScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/production/scalability');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Évolutivité</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ProdScalabilityScreenDetail', { id: item.id })}>
