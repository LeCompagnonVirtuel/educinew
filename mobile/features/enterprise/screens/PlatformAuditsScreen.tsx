...
export const PlatformAuditsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/platform-audits');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Audits de la plateforme</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('PlatformAuditsScreenDetail', { id: item.id })}>
