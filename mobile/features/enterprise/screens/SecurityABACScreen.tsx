...
export const SecurityABACScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/security/abac');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Contrôle d'accès basé sur les attributs</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('SecurityABACScreenDetail', { id: item.id })}>
