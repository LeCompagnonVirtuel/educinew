...
export const ObservabilityTracingScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/enterprise/observability/tracing');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Traçage distribué</Text>
      {items.map((item) => (
        <TouchableOpacity key={item.id} style={styles.card}
          onPress={() => navigation.navigate('ObservabilityTracingScreenDetail', { id: item.id })}>
