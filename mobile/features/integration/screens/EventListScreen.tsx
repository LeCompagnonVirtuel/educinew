      ...
      {events.map((event) => (
        <View key={event.id} style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <View style={[styles.statusBadge, { backgroundColor: event.status === 'processed' ? '#4CAF50' : '#FF9800' }]}>
              <Text style={styles.statusText}>{event.status}</Text>
            </View>
          </View>
          <Text style={styles.cardSource}>Source: {event.source}</Text>
...
const styles = StyleSheet.create({
  ...
  cardSource: { fontSize: 14, color: '#666', marginTop: 8 },
  cardPayload: { fontSize: 14, color: '#888', marginTop: 4, fontFamily: 'monospace' },
