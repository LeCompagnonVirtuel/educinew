...
export default function IntSentimentScreen({ navigation }: any) {
  const [items, setItems] = useState<IntSentiment[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntSentimentScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucune analyse de sentiment trouvée</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntSentimentDetail', { id: item.id })}>
