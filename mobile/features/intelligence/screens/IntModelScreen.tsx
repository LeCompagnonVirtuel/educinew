...
export default function IntModelScreen({ navigation }: any) {
  const [items, setItems] = useState<IntModel[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntModelScreen]', err);
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntModelDetail', { id: item.id })}>
