...
export default function IntClassificationScreen({ navigation }: any) {
  const [items, setItems] = useState<IntClassification[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntClassificationScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucune classification trouvée</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntClassificationDetail', { id: item.id })}>
