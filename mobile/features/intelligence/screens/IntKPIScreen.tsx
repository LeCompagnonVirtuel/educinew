...
export default function IntKPIScreen({ navigation }: any) {
  const [items, setItems] = useState<IntKPI[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntKPIScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun KPI trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntKPIDetail', { id: item.id })}>
