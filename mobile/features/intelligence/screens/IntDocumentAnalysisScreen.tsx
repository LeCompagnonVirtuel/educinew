...
export default function IntDocumentAnalysisScreen({ navigation }: any) {
  const [items, setItems] = useState<IntDocumentAnalysis[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntDocumentAnalysisScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucune analyse de document trouvée</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntDocumentAnalysisDetail', { id: item.id })}>
