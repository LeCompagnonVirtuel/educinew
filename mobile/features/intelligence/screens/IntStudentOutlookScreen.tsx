...
export default function IntStudentOutlookScreen({ navigation }: any) {
  const [items, setItems] = useState<IntStudentOutlook[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntStudentOutlookScreen]', err);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Student Outlook</Text>
        <TouchableOpacity>
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucune perspective étudiante trouvée</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntStudentOutlookDetail', { id: item.id })}>
