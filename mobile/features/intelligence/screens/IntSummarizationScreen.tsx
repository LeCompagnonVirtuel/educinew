...
export default function IntSummarizationScreen({ navigation }: any) {
  const [items, setItems] = useState<IntSummarization[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntSummarizationScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucune résumé trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntSummarizationDetail', { id: item.id })}>
