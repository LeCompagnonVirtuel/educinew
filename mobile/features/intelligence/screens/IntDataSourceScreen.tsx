...
export default function IntDataSourceScreen({ navigation }: any) {
  const [items, setItems] = useState<IntDataSource[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntDataSourceScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucune source de données trouvée</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntDataSourceDetail', { id: item.id })}>
