...
export default function IntVisualizationScreen({ navigation }: any) {
  const [items, setItems] = useState<IntVisualization[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntVisualizationScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucune visualisation trouvée</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntVisualizationDetail', { id: item.id })}>
