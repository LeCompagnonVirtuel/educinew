...
export default function IntPipelineScreen({ navigation }: any) {
  const [items, setItems] = useState<IntPipeline[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntPipelineScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun pipeline trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntPipelineDetail', { id: item.id })}>
