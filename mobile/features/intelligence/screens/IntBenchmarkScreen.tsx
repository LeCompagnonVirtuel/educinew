...
export default function IntBenchmarkScreen({ navigation }: any) {
  const [items, setItems] = useState<IntBenchmark[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntBenchmarkScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun benchmark trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntBenchmarkDetail', { id: item.id })}>
