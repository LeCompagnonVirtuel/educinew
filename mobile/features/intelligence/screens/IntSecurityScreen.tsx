...
export default function IntSecurityScreen({ navigation }: any) {
  const [items, setItems] = useState<IntSecurity[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntSecurityScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucune sécurité trouvée</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntSecurityDetail', { id: item.id })}>
