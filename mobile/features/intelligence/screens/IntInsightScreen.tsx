...
export default function IntInsightScreen({ navigation }: any) {
  const [items, setItems] = useState<IntInsight[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntInsightScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun insight trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntInsightDetail', { id: item.id })}>
