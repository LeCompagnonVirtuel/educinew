...
export default function IntScoreScreen({ navigation }: any) {
  const [items, setItems] = useState<IntScore[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntScoreScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun score trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntScoreDetail', { id: item.id })}>
