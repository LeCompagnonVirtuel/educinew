...
export default function IntAIInsightScreen({ navigation }: any) {
  const [items, setItems] = useState<IntAIInsight[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntAIInsightScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun insight IA trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntAIInsightDetail', { id: item.id })}>
