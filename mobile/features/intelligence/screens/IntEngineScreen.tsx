...
export default function IntEngineScreen({ navigation }: any) {
  const [items, setItems] = useState<IntEngine[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntEngineScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun engine trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntEngineDetail', { id: item.id })}>
