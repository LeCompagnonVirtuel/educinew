...
export default function IntKnowledgeBaseScreen({ navigation }: any) {
  const [items, setItems] = useState<IntKnowledgeBase[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntKnowledgeBaseScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucune base de connaissances trouvée</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntKnowledgeBaseDetail', { id: item.id })}>
