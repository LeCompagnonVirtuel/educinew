...
export default function IntScenarioScreen({ navigation }: any) {
  const [items, setItems] = useState<IntScenario[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntScenarioScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun scénario trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntScenarioDetail', { id: item.id })}>
