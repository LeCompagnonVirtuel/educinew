...
export default function IntMonitoringScreen({ navigation }: any) {
  const [items, setItems] = useState<IntMonitoring[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntMonitoringScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun monitoring trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntMonitoringDetail', { id: item.id })}>
