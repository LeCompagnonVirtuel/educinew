...
export default function IntNLPTaskScreen({ navigation }: any) {
  const [items, setItems] = useState<IntNLPTask[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntNLPTaskScreen]', err);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>NLP Task</Text>
        <TouchableOpacity>
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucune tâche NLP trouvée</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntNLPTaskDetail', { id: item.id })}>
