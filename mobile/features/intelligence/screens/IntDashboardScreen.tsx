...
export default function IntDashboardScreen({ navigation }: any) {
  const [items, setItems] = useState<IntDashboard[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntDashboardScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun dashboard trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntDashboardDetail', { id: item.id })}>
