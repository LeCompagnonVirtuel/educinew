...
export default function IntActionScreen({ navigation }: any) {
  const [items, setItems] = useState<IntAction[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntActionScreen]', err);
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntActionDetail', { id: item.id })}>
