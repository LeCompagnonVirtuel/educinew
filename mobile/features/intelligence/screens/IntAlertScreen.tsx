...
export default function IntAlertScreen({ navigation }: any) {
  const [items, setItems] = useState<IntAlert[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntAlertScreen]', err);
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntAlertDetail', { id: item.id })}>
