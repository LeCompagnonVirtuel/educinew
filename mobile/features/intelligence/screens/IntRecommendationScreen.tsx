...
export default function IntRecommendationScreen({ navigation }: any) {
  const [items, setItems] = useState<IntRecommendation[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntRecommendationScreen]', err);
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntRecommendationDetail', { id: item.id })}>
