...
export default function IntDataSyncScreen({ navigation }: any) {
  const [items, setItems] = useState<IntDataSync[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntDataSyncScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucune synchronisation de données trouvée</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntDataSyncDetail', { id: item.id })}>
