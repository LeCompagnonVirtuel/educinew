...
export default function IntStudentRiskScreen({ navigation }: any) {
  const [items, setItems] = useState<IntStudentRisk[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntStudentRiskScreen]', err);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Student Risk</Text>
        <TouchableOpacity>
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun risque étudiant trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntStudentRiskDetail', { id: item.id })}>
