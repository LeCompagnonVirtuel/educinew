...
export default function IntAPIScreen({ navigation }: any) {
  const [items, setItems] = useState<IntAPI[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntAPIScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucune API trouvée</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntAPIDetail', { id: item.id })}>
