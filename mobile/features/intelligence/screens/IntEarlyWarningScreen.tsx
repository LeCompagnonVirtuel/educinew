...
export default function IntEarlyWarningScreen({ navigation }: any) {
  const [items, setItems] = useState<IntEarlyWarning[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntEarlyWarningScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun avertissement précoce trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntEarlyWarningDetail', { id: item.id })}>
