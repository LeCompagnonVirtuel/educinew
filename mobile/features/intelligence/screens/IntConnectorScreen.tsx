...
export default function IntConnectorScreen({ navigation }: any) {
  const [items, setItems] = useState<IntConnector[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntConnectorScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun connecteur trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntConnectorDetail', { id: item.id })}>
