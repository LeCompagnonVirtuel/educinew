...
export default function IntWidgetScreen({ navigation }: any) {
  const [items, setItems] = useState<IntWidget[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntWidgetScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun widget trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntWidgetDetail', { id: item.id })}>
