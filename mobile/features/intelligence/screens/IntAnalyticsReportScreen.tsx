...
export default function IntAnalyticsReportScreen({ navigation }: any) {
  const [items, setItems] = useState<IntAnalyticsReport[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntAnalyticsReportScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun rapport analytique trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntAnalyticsReportDetail', { id: item.id })}>
