...
export default function IntPredictiveModelScreen({ navigation }: any) {
  const [items, setItems] = useState<IntPredictiveModel[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error('[IntPredictiveModelScreen]', err);
      ...
      >
        ...
        {items.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Aucun modèle prédictif trouvé</Text>
          </View>
        ) : (
          items.map((item) => (
            <TouchableOpacity key={item.id} onPress={() => navigation.navigate('IntPredictiveModelDetail', { id: item.id })}>
