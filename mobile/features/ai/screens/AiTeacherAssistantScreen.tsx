...
export const AiTeacherAssistantScreen: React.FC<{ navigation: { navigate: (screen: string, params?: Record<string, unknown>) => void } }> = ({ navigation }) => {
  const [loading, setLoading] = useState(true);
  const [features, setFeatures] = useState<TeacherFeature[]>([]);
  ...
  const fetchFeatures = async () => {
    try {
      const response = await fetch('/api/ai/teacher-assistant/features');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Assistant Enseignant</Text>
      <Text style={styles.subtitle}>Outils intelligents pour vos cours</Text>

      <View style={styles.welcomeCard}>
        <Text style={styles.welcomeTitle}>Bienvenue !</Text>
        <Text style={styles.welcomeText}>Optimisez votre enseignement avec l'IA</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  welcomeCard: { backgroundColor: '#28a745', borderRadius: 12, padding: 20, marginBottom: 20 },
