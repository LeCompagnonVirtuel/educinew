...
export const AiStudentAssistantScreen: React.FC<{ navigation: { navigate: (screen: string, params?: Record<string, unknown>) => void } }> = ({ navigation }) => {
  ...
  const fetchFeatures = async () => {
    try {
      const response = await fetch('/api/ai/student-assistant/features');
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Assistant Élève</Text>
      <Text style={styles.subtitle}>Votre compagnon d'apprentissage intelligent</Text>

      <View style={styles.welcomeCard}>
        <Text style={styles.welcomeTitle}>Bonjour ! 👋</Text>
        <Text style={styles.welcomeText}>Comment puis-je vous aider aujourd'hui ?</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  welcomeCard: { backgroundColor: '#1565c0', borderRadius: 12, padding: 20, marginBottom: 20 },
