-- Migration: Fix enterprise_activate_school to handle scalar JSON values
-- The modules/cycles/subjects might be stored as scalar strings instead of arrays
...
BEGIN
  ...
  BEGIN
    -- Safely parse JSON arrays (handle both array and scalar values)
    BEGIN
      IF v_draft.modules IS NULL THEN
        v_modules := '[]'::jsonb;
      ELSIF jsonb_typeof(v_draft.modules) = 'array' THEN
        v_modules := v_draft.modules;
      ELSE
        -- Try to parse as JSON string, fallback to empty array
        BEGIN
    ...
    BEGIN
      IF v_draft.academic_cycles IS NULL THEN
        v_cycles := '[]'::jsonb;
      ELSIF jsonb_typeof(v_draft.academic_cycles) = 'array' THEN
        v_cycles := v_draft.academic_cycles;
      ELSE
        BEGIN
          v_cycles := v_draft.academic_cycles::text::jsonb;
          IF jsonb_typeof(v_cycles) != 'array' THEN
            v_cycles := '[]'::jsonb;
          END IF;
        EXCEPTION WHEN OTHERS THEN
          v_cycles := '[]'::jsonb;
    ...
    BEGIN
      IF v_draft.academic_subjects IS NULL THEN
        v_subjects := '[]'::jsonb;
      ELSIF jsonb_typeof(v_draft.academic_subjects) = 'array' THEN
        v_subjects := v_draft.academic_subjects;
      ELSE
        BEGIN
    ...
    SELECT id INTO v_existing_school_id
    FROM schools
    WHERE email = COALESCE(v_draft.contact_email_primary, v_draft.owner_email)
    LIMIT 1;

    IF v_existing_school_id IS NOT NULL THEN
      v_school_id := v_existing_school_id;
      UPDATE schools SET
        name = v_draft.school_official_name,
        address = v_draft.location_full_address,
        phone = v_draft.contact_phone_primary,
        region = v_draft.location_region,
        city = v_draft.location_city,
        country = COALESCE(v_draft.location_country, 'Côte d''Ivoire'),
        school_type = v_draft.school_type,
        visibility = v_draft.school_visibility,
        commercial_name = v_draft.school_commercial_name,
        acronym = v_draft.school_acronym,
        sigle = v_draft.school_acronym,
        ministry = v_draft.school_ministry,
        rccm = v_draft.school_rccm,
        fiscal_number = v_draft.school_fiscal_number,
        authorization_number = v_draft.school_authorization_number,
        creation_date = v_draft.school_creation_date,
        commune = v_draft.location_commune,
        quarter = v_draft.location_quarter,
        postal_code = v_draft.location_postal_code,
        latitude = v_draft.location_latitude::REAL,
        longitude = v_draft.location_longitude::REAL,
        website = v_draft.contact_website,
        description = v_draft.branding_description,
        slogan = v_draft.branding_slogan,
        logo_url = v_draft.branding_logo_url,
        ...
        facebook = v_draft.contact_facebook,
        linkedin = v_draft.contact_linkedin,
        instagram = v_draft.contact_instagram,
        youtube = v_draft.contact_youtube,
        twitter = v_draft.contact_twitter,
        is_active = true,
        updated_at = now()
    ...
    -- 9. Create school modules (safely handle array)
    IF jsonb_array_length(v_modules) > 0 THEN
      ...
      FROM jsonb_array_elements_text(v_modules) AS module_name
    ...
    -- 13. Create academic structure (safely handle arrays)
    IF jsonb_array_length(v_cycles) > 0 THEN
      INSERT INTO cycles (school_id, name)
      SELECT v_school_id, cycle_name
      FROM jsonb_array_elements_text(v_cycles) AS cycle_name
      ON CONFLICT DO NOTHING;
    END IF;
    ...
    VALUES (p_draft_id, 'school_activated', jsonb_build_object('school_id', v_school_id, 'school_code', v_school_code, 'user_id', v_user_id, 'reused_existing', v_existing_school_id IS NOT NULL));
