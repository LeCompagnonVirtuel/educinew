-- Migration: Fix enterprise_activate_school to handle duplicate school email
-- If a school already exists with that email, reuse it instead of creating a new one
...
BEGIN
  ...
  BEGIN
    ...
    -- 3. Check if school already exists (from previous failed attempt)
    ...
    IF v_existing_school_id IS NOT NULL THEN
      -- School already exists, update it
