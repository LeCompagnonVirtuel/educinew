...
serve(async (req) => {
  ...
  try {
    ...
    if (!content || typeof content !== "string" || content.length > 10000) {
      return new Response(JSON.stringify({ error: "Contenu invalide (max 10000 caractères)" }), {
        status: 400,
        headers: { ...headers, "Content-Type": "application/json" },
      });
    }
    ...
    Résume le contenu suivant de manière claire et structurée.
    Utilise des points clés, des titres et des sous-titres.
    Mets en évidence les informations importantes.
    Adapte le résumé au niveau scolaire ouest-africain.`;
    ...
    const response = await fetch("https://api.deepseek.com/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: `Résume cette leçon de ${safeSubject}:\n\n${content}` },
        ],
        temperature: 0.7,
        max_tokens: 2000,
      }),
    ...
    const reply = data.choices?.[0]?.message?.content || "Désolé, je n'ai pas pu résumer ce contenu.";

    return new Response(JSON.stringify({ response: reply }), {
      headers: { ...headers, "Content-Type": "application/json", "X-RateLimit-Remaining": String(remaining) },
    });
  } catch (error) {
    console.error("[ai-summarize] Fatal error:", error);
